...
static bool printtup(TupleTableSlot *tupleTableSlot, DestReceiver *destReceiver);
...
printtup_prepare_info(DR_printtup *myState, TupleDesc typeinfo, int numAttrs) {
    ...
    for (i = 0; i < numAttrs; i++) {
        ...
        if (format == 0) {
            ...
            ereport(ERROR,
                    (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("unsupported format code: %d", format)));
    }
}

// print a tuple in protocol 3.0
static bool printtup(TupleTableSlot *tupleTableSlot, DestReceiver *destReceiver) {
    TupleDesc tupleDesc = tupleTableSlot->tts_tupleDescriptor;
    DR_printtup *myState = (DR_printtup *) destReceiver;

    StringInfo buf = &myState->buf;
    int natts = tupleDesc->natts;

    /* Set or update my derived attribute info, if needed */
    if (myState->attrinfo != tupleDesc || myState->nattrs != natts)
        printtup_prepare_info(myState, tupleDesc, natts);
    ...
    // send the attribute of this tuple
    for (int i = 0; i < natts; ++i) {
        PrinttupAttrInfo *thisState = myState->myinfo + i;
        Datum attr = tupleTableSlot->tts_values[i];

        if (tupleTableSlot->tts_isnull[i]) {
            pq_sendint32(buf, -1);
            continue;
        }
