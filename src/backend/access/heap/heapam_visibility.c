 ...
 */
static bool HeapTupleSatisfiesMVCC(HeapTuple heapTuple, Snapshot snapshot, Buffer buffer) {
    ...
    if (!HeapTupleHeaderXminCommitted(tuple)) { // xmin不是committed,很有可能invisible
        ...
        } else if (tuple->t_infomask & HEAP_MOVED_IN) { // Used by pre-9.0 binary upgrade
            TransactionId xvac = HeapTupleHeaderGetXvac(tuple);
...
HeapTupleSatisfiesHistoricMVCC(HeapTuple htup, Snapshot snapshot,
    ...
    if (TransactionIdFollowsOrEquals(xmax, snapshot->xmax))
        return true;
    /* xmax is between [xmin, xmax), check known committed array */
    if (TransactionIdInArray(xmax, snapshot->xip, snapshot->xcnt))
        return false;
 ...
 */
bool HeapTupleSatisfiesVisibility(HeapTuple heapTuple,
                                  Snapshot snapshot,
                                  Buffer buffer) {
    switch (snapshot->snapshot_type) {
        case SNAPSHOT_MVCC:
            return HeapTupleSatisfiesMVCC(heapTuple, snapshot, buffer);
        case SNAPSHOT_SELF:
            return HeapTupleSatisfiesSelf(heapTuple, snapshot, buffer);
        case SNAPSHOT_ANY:
            return HeapTupleSatisfiesAny(heapTuple, snapshot, buffer);
        case SNAPSHOT_TOAST:
            return HeapTupleSatisfiesToast(heapTuple, snapshot, buffer);
        case SNAPSHOT_DIRTY:
            return HeapTupleSatisfiesDirty(heapTuple, snapshot, buffer);
        case SNAPSHOT_HISTORIC_MVCC:
            return HeapTupleSatisfiesHistoricMVCC(heapTuple, snapshot, buffer);
