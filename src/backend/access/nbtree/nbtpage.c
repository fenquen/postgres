...
/*
 * release a locked buffer and get another one.
 *
 ...
 */
Buffer _bt_relandgetbuf(Relation indexRelation,
                        Buffer oldBuffer,
                        BlockNumber targetBlockNumber,
                        int lockMode) {
    Assert(targetBlockNumber != P_NEW);

    if (BufferIsValid(oldBuffer)) {
        LockBuffer(oldBuffer, BUFFER_LOCK_UNLOCK);
    }

    Buffer buffer = ReleaseAndReadBuffer(oldBuffer, indexRelation, targetBlockNumber);
    LockBuffer(buffer, lockMode);

    _bt_checkpage(indexRelation, buffer);
