...
bool btgettuple(IndexScanDesc scan, ScanDirection dir) {
     ...
     */
    if (btScanOpaque->numArrayKeys && !BTScanPosIsValid(btScanOpaque->currPos)) {
        /* punt if we have any unsatisfiable array keys */
        if (btScanOpaque->numArrayKeys < 0) {
            return false;
        }
    ...
    do {
        ...
        if (!BTScanPosIsValid(btScanOpaque->currPos))
            ...
            if (scan->kill_prior_tuple) {
                 ...
                 */
                if (btScanOpaque->killedItems == NULL)
                    btScanOpaque->killedItems = (int *) palloc(MaxIndexTuplesPerPage * sizeof(int));

                if (btScanOpaque->numKilled < MaxIndexTuplesPerPage)
...
IndexScanDesc btbeginscan(Relation rel, int nkeys, int norderbys) {
    ...
    /* get the scan */
    IndexScanDesc indexScanDesc = RelationGetIndexScan(rel, nkeys, norderbys);

    /* allocate private workspace */
    BTScanOpaque btScanOpaque = (BTScanOpaque) palloc(sizeof(BTScanOpaqueData));
    BTScanPosInvalidate(btScanOpaque->currPos);
    BTScanPosInvalidate(btScanOpaque->markPos);
    if (indexScanDesc->numberOfKeys > 0) {
        btScanOpaque->keyData = (ScanKey) palloc(indexScanDesc->numberOfKeys * sizeof(ScanKeyData));
    } else {
        btScanOpaque->keyData = NULL;
    }
    btScanOpaque->arrayKeyData = NULL;    /* assume no array keys for now */
    btScanOpaque->numArrayKeys = 0;
    btScanOpaque->arrayKeys = NULL;
    btScanOpaque->arrayContext = NULL;
    btScanOpaque->killedItems = NULL;        /* until needed */
    btScanOpaque->numKilled = 0;
     ...
     */
    btScanOpaque->currTuples = btScanOpaque->markTuples = NULL;

    indexScanDesc->xs_itupdesc = RelationGetDescr(rel);
    indexScanDesc->opaque = btScanOpaque;
