...
char *temp_tablespaces = NULL; // 逗号分割的
...
void PrepareTempTablespaces(void) {
    ...
    /* Parse string into list of identifiers */
    List *tablespaceNameList;
    if (!SplitIdentifierString(rawname, ',', &tablespaceNameList)) {
        /* syntax error in name list */
        SetTempTablespaces(NULL, 0);
        pfree(rawname);
        list_free(tablespaceNameList);
        return;
    }

    /* Store tablespace OIDs in an array in TopTransactionContext */
    Oid *tablespaceOids = (Oid *) MemoryContextAlloc(TopTransactionContext,
                                                     list_length(tablespaceNameList) * sizeof(Oid));
    int tablespaceNum = 0;

    ListCell *listCell;
    foreach(listCell, tablespaceNameList) {
        char *tablespaceName = (char *) lfirst(listCell);


        /* Allow an empty string (signifying database default) */
        if (tablespaceName[0] == '\0') {
            /* InvalidOid signifies database's default tablespace */
            tablespaceOids[tablespaceNum++] = InvalidOid;
            continue;
        }

        /* Else verify that name is a valid tablespace name */
        Oid oid = get_tablespace_oid(tablespaceName, true);
        if (oid == InvalidOid) {
        ...
        // 当前用户有没有权限
        AclResult aclresult = pg_tablespace_aclcheck(oid, GetUserId(),
                                                     ACL_CREATE);
        if (aclresult != ACLCHECK_OK) {
            continue;
        }

        tablespaceOids[tablespaceNum++] = oid;
    }

    SetTempTablespaces(tablespaceOids, tablespaceNum);
