...
ExprState *ExecInitExpr(Expr *expr, PlanState *parent) {
    ...
    ExprState *exprState = makeNode(ExprState);
    exprState->expr = expr;
    exprState->parent = parent;
    exprState->ext_params = NULL;

    // Insert EEOP_*_FETCHSOME steps as needed */
    ExecInitExprSlots(exprState, (Node *) expr);

    // Compile the expression proper */
    ExecInitExprRec(expr, exprState, &exprState->resvalue, &exprState->resnull);

    // append a EEOP_DONE step
    ExprEvalStep exprEvalStep = {0};
    exprEvalStep.opcode = EEOP_DONE;
    ExprEvalPushStep(exprState, &exprEvalStep);
...
// call on a list of expressions, return a list of ExprStates.
List *ExecInitExprList(List *exprList, PlanState *parent) {
    List *result = NIL;

    ListCell *listCell;
    foreach(listCell, exprList) {
        Expr *expr = lfirst(listCell);
        result = lappend(result, ExecInitExpr(expr, parent));
    }
 ...
 */
void ExprEvalPushStep(ExprState *exprState, const ExprEvalStep *exprEvalStep) {
    if (exprState->steps_alloc == 0) {
        exprState->steps_alloc = 16;
        exprState->steps = palloc(sizeof(ExprEvalStep) * exprState->steps_alloc);
    } else if (exprState->steps_alloc == exprState->steps_len) {
        exprState->steps_alloc *= 2;
        exprState->steps = repalloc(exprState->steps, sizeof(ExprEvalStep) * exprState->steps_alloc);
    }

    memcpy(&exprState->steps[exprState->steps_len++], exprEvalStep, sizeof(ExprEvalStep));
}
