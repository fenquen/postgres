...
static Datum ExecJustOuterVar(ExprState *exprState, ExprContext *exprContext, bool *isnull);
...
static Datum ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull) {
    ...
    EEO_SWITCH()
    {
        ...
        EEO_CASE(EEOP_FUNCEXPR_STRICT)
        {
            ...
            fcinfo->isnull = false;
            Datum d = op->d.func.fn_addr(fcinfo); // 调用texteq()
            *op->resvalue = d;
            *op->resnull = fcinfo->isnull;
...
static Datum ExecJustOuterVar(ExprState *exprState, ExprContext *exprContext, bool *isnull) {
    ExprEvalStep *exprEvalStep = &exprState->steps[1];
    int attnum = exprEvalStep->d.var.attnum + 1;

    TupleTableSlot *tupleTableSlot = exprContext->ecxt_outertuple; // 对应了outer表 test_table 的data

    CheckOpSlotCompatibility(&exprState->steps[0], tupleTableSlot);

    /* See comments in ExecJustInnerVar */
    return slot_getattr(tupleTableSlot, attnum, isnull);
}
