 ...
 * ----------------------------------------------------------------
 */
HashJoinTable ExecHashTableCreate(HashState *hashState,
                                  List *hashOperators,
                                  List *hashCollations,
                                  bool keepNulls) {
     ...
     */
    double rows = hash->plan.parallel_aware ? hash->rows_total : planOuter->plan_rows;
    ...
    ExecChooseHashTableSize(rows,
                            ...
                            hashState->parallel_state != NULL ? hashState->parallel_state->nparticipants - 1 : 0,
    ...
    Assert(bucketCount == (1 << log2BucketNum));
     ...
     */
    HashJoinTable hashJoinTable = (HashJoinTable) palloc(sizeof(HashJoinTableData));
    hashJoinTable->nbuckets = bucketCount;
    hashJoinTable->nbuckets_original = bucketCount;
    hashJoinTable->nbuckets_optimal = bucketCount;
    hashJoinTable->log2_nbuckets = log2BucketNum;
    hashJoinTable->log2_nbuckets_optimal = log2BucketNum;
    hashJoinTable->buckets.unshared = NULL;
    hashJoinTable->keepNulls = keepNulls;
    hashJoinTable->skewEnabled = false;
    hashJoinTable->skewBucket = NULL;
    hashJoinTable->skewBucketLen = 0;
    hashJoinTable->nSkewBuckets = 0;
    hashJoinTable->skewBucketNums = NULL;
    hashJoinTable->nbatch = batchCount;
    hashJoinTable->curbatch = 0;
    hashJoinTable->nbatch_original = batchCount;
    hashJoinTable->nbatch_outstart = batchCount;
    hashJoinTable->growEnabled = true;
    hashJoinTable->totalTuples = 0;
    hashJoinTable->partialTuples = 0;
    hashJoinTable->skewTuples = 0;
    hashJoinTable->innerBatchFile = NULL;
    hashJoinTable->outerBatchFile = NULL;
    hashJoinTable->spaceUsed = 0;
    hashJoinTable->spacePeak = 0;
    hashJoinTable->spaceAllowed = spaceByteCountAllowed;
    hashJoinTable->spaceUsedSkew = 0;
    hashJoinTable->spaceAllowedSkew = hashJoinTable->spaceAllowed * SKEW_WORK_MEM_PERCENT / 100;
    hashJoinTable->chunks = NULL;
    hashJoinTable->current_chunk = NULL;
    hashJoinTable->parallel_state = hashState->parallel_state;
    hashJoinTable->area = hashState->ps.state->es_query_dsa;
    hashJoinTable->batches = NULL;
    ...
    // Create temporary memory contexts in which to keep the hashtable working storage
    // See notes in executor/hashjoin.h.
    hashJoinTable->hashCxt = AllocSetContextCreate(CurrentMemoryContext, "HashTableContext", ALLOCSET_DEFAULT_SIZES);
    ...
    hashJoinTable->outer_hashfunctions = (FmgrInfo *) palloc(nkeys * sizeof(FmgrInfo));
    hashJoinTable->inner_hashfunctions = (FmgrInfo *) palloc(nkeys * sizeof(FmgrInfo));
    hashJoinTable->hashStrict = (bool *) palloc(nkeys * sizeof(bool));
    hashJoinTable->collations = (Oid *) palloc(nkeys * sizeof(Oid));
    ...
    forboth(ho, hashOperators, hc, hashCollations) {
        ...
        if (!get_op_hash_functions(hashop, &left_hashfn, &right_hashfn))
            elog(ERROR, "could not find hash function for hash operator %u",
                 hashop);
        fmgr_info(left_hashfn, &hashJoinTable->outer_hashfunctions[i]);
        fmgr_info(right_hashfn, &hashJoinTable->inner_hashfunctions[i]);
        hashJoinTable->hashStrict[i] = op_strict(hashop);
        hashJoinTable->collations[i] = lfirst_oid(hc);
        i++;
    }

    if (batchCount > 1 && hashJoinTable->parallel_state == NULL) {
        // allocate and initialize the file arrays in hashCxt (not needed for
        // parallel case which uses shared tuplestores instead of raw files)
        hashJoinTable->innerBatchFile = (BufFile **) palloc0(batchCount * sizeof(BufFile *));
        hashJoinTable->outerBatchFile = (BufFile **) palloc0(batchCount * sizeof(BufFile *));
    ...
    if (hashJoinTable->parallel_state) {
         ...
         */
        Barrier *build_barrier = &parallelHashJoinState->build_barrier;
        ...
        if (BarrierPhase(build_barrier) == PHJ_BUILD_ELECTING &&
            ...
            parallelHashJoinState->nbatch = batchCount;
            parallelHashJoinState->space_allowed = spaceByteCountAllowed;
            parallelHashJoinState->growth = PHJ_GROWTH_OK;

            /* Set up the shared state for coordinating batches. */
            ExecParallelHashJoinSetUpBatches(hashJoinTable, batchCount);

            // allocate batch 0's hash table up front so we can load it directly while hashing.
            parallelHashJoinState->nbuckets = bucketCount;
            ExecParallelHashTableAlloc(hashJoinTable, 0);
        }
         ...
         */
        MemoryContextSwitchTo(hashJoinTable->batchCxt);

        hashJoinTable->buckets.unshared = (HashJoinTuple *) palloc0(bucketCount * sizeof(HashJoinTuple));
...
void ExecChooseHashTableSize(double tupleCount,
                             int tupleWidth, // 单个的tuple的byte数量
                             bool useSkew,
                             bool parallelHash,
    ...
    double innerRelByteCount = tupleCount * tupsize;

    // 允许的in memory的hash table大小
    long hashTableInMemoryByteLimit = work_mem * 1024L;
     ...
     */
    if (parallelHash) {
        hashTableInMemoryByteLimit += hashTableInMemoryByteLimit * parallel_workers;
    }

    *spaceByteCountAllowed = hashTableInMemoryByteLimit;
     ...
     */
    if (useSkew) {
        long skewTableByteCount = hashTableInMemoryByteLimit * SKEW_WORK_MEM_PERCENT / 100;
         ...
         *----------
         */
        *skewMcvCount = skewTableByteCount / (tupsize +
                                              (8 * sizeof(HashSkewBucket *)) +
                                              sizeof(int) +
                                              SKEW_BUCKET_OVERHEAD);
        if (*skewMcvCount > 0) {
            hashTableInMemoryByteLimit -= skewTableByteCount;
        }
    } else {
        *skewMcvCount = 0;
    }
     ...
     * the pointer arrays we'll try to allocate do not exceed work_mem nor MaxAllocSize.
     *
     * Note that both nbuckets and nbatch must be powers of 2 to make
     * ExecHashGetBucketAndBatch fast.
     */
    long maxPointerCount = *spaceByteCountAllowed / sizeof(HashJoinTuple);
    maxPointerCount = Min(maxPointerCount, MaxAllocSize / sizeof(HashJoinTuple));

    /* If max_pointers isn't a power of 2, must round it down to one */
    mppow2 = 1L << my_log2(maxPointerCount);
    if (maxPointerCount != mppow2) {
        maxPointerCount = mppow2 / 2;
    }

    // Also ensure we avoid integer overflow in nbatch and nbuckets */
    // (this step is redundant given the current value of MaxAllocSize) */
    maxPointerCount = Min(maxPointerCount, INT_MAX / 2);

    double bucketCountDouble = ceil(tupleCount / NTUP_PER_BUCKET);
    bucketCountDouble = Min(bucketCountDouble, maxPointerCount);

    int bucketCount_ = (int) bucketCountDouble;
    /* don't let nbuckets be really small, though ... */
    bucketCount_ = Max(bucketCount_, 1024);
    /* ... and force it to be a power of 2. */
    bucketCount_ = 1 << my_log2(bucketCount_);
     ...
     */
    long totalBucketByteCount = sizeof(HashJoinTuple) * bucketCount_;
    if (innerRelByteCount + totalBucketByteCount > hashTableInMemoryByteLimit) { // we'll need multiple batches
         ...
         */
        if (parallelHash) {
            ExecChooseHashTableSize(tupleCount, tupleWidth, useSkew,
         ...
         */
        long singleBucketByteCount = tupsize * NTUP_PER_BUCKET + sizeof(HashJoinTuple);

        long bucketCountLong = 1L << my_log2(hashTableInMemoryByteLimit / singleBucketByteCount);
        bucketCountLong = Min(bucketCountLong, maxPointerCount);

        bucketCount_ = (int) bucketCountLong;
        bucketCount_ = 1 << my_log2(bucketCount_);
        totalBucketByteCount = sizeof(HashJoinTuple) * bucketCount_;
         ...
         * look for 50% here.
         */
        Assert(totalBucketByteCount <= hashTableInMemoryByteLimit / 2);

        /* Calculate required number of batches. */
        double batchCoubtDouble = ceil(innerRelByteCount / (hashTableInMemoryByteLimit - totalBucketByteCount));
        batchCoubtDouble = Min(batchCoubtDouble, maxPointerCount);

        batchCount_ = 2;
        while (batchCount_ < (int) batchCoubtDouble)
            batchCount_ <<= 1;
    }

    Assert(bucketCount_ > 0);
    Assert(batchCount_ > 0);
