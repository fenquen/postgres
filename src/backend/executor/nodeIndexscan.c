...
static TupleTableSlot *IndexNext(IndexScanState *indexScanState) {
    ...
    if (ScanDirectionIsBackward(((IndexScan *) indexScanState->ss.ps.plan)->indexorderdir)) {
        ...
        } else if (ScanDirectionIsBackward(scanDirection)) {
    ...
    if (indexScanDesc == NULL) {
         ...
         */
        indexScanDesc = index_beginscan(indexScanState->ss.ss_currentRelation,
                                        indexScanState->iss_RelationDesc,
                                        estate->es_snapshot,
                                        indexScanState->iss_NumScanKeys,
                                        indexScanState->iss_NumOrderByKeys);

        indexScanState->iss_ScanDesc = indexScanDesc;
    ...
    while (index_getnext_slot(indexScanDesc, scanDirection, tupleTableSlot)) {
        ...
        if (indexScanDesc->xs_recheck) {
            ...
            if (!ExecQualAndReset(indexScanState->indexqualorig, exprContext)) {
                ...
                InstrCountFiltered2(indexScanState, 1);
                continue;
            }
        }
    ...
    // here means the index scan failed so we are at the end of the scan
    indexScanState->iss_ReachedEnd = true;
    return ExecClearTuple(tupleTableSlot);
}
 ...
 * ----------------------------------------------------------------
 */
static TupleTableSlot *ExecIndexScan(PlanState *planState) {
    IndexScanState *indexScanState = castNode(IndexScanState, planState);

    // If we have runtime keys and they've not already been set up, do it now.
    if (indexScanState->iss_NumRuntimeKeys != 0 && !indexScanState->iss_RuntimeKeysReady) {
        ExecReScan((PlanState *) indexScanState);
    }

    if (indexScanState->iss_NumOrderByKeys > 0) {
        return ExecScan(&indexScanState->ss,
