...
int StreamServerPort(int family,
                     ...
                     int MaxListen) { // MaxListen 最多能listen的地址个数
	pgsocket	fd;
	int			err;
	int			maxconn;
	int			ret;
	...
	char	   *service;
	struct addrinfo *addrinfoArr = NULL,
			         *addr;
    ...
    // 通过hostname 和 port得到多个addrinfo 灌入到addrinfoArr
	ret = pg_getaddrinfo_all(hostName, service, &hint, &addrinfoArr);
	if (ret || !addrinfoArr) {
		if (hostName) {
            ereport(LOG,
		...
		if (addrinfoArr) {
			pg_freeaddrinfo_all(hint.ai_family, addrinfoArr);
        }
    ...
    // 遍历得到的成果
    for (addr = addrinfoArr; addr; addr = addr->ai_next) {
        ...
        // 找到当前还有的空余的位置 其值的标记是PGINVALID_SOCKET
...
#endif
            default:
                snprintf(familyDescBuf, sizeof(familyDescBuf), _("unrecognized address family %d"),
                         addr->ai_family);
         ...
         * ipv4 addresses to ipv6.  It will show ::ffff:ipv4 for all ipv4 connections.
         */
