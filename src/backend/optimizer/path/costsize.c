...
/*
 * 计算 path的 startup_cost / total_cost / rows
 *	  Determines and returns the cost of scanning a function rangeTblEntry(其rtekind是RTE_FUNCTION).
 *
 * 'baserel' is the relation to be scanned
 * 'param_info' is the ParamPathInfo if this is a parameterized path, else NULL
 */
void cost_functionscan(Path *path,
                       PlannerInfo *root,
                       RelOptInfo *baseRelOptInfo,
                       ParamPathInfo *paramPathInfo) {

    /* Should only be applied to base relations that are functions */
    Assert(baseRelOptInfo->relid > 0);
    RangeTblEntry *rangeTblEntry = planner_rt_fetch(baseRelOptInfo->relid, root);
    Assert(rangeTblEntry->rtekind == RTE_FUNCTION);

    if (paramPathInfo)
        path->rows = paramPathInfo->ppi_rows;
    else
        path->rows = baseRelOptInfo->rows;
	 ...
	 */
    QualCost exprCost;
    cost_qual_eval_node(&exprCost, (Node *) rangeTblEntry->functions, root);

    Cost startup_cost = 0;
    startup_cost += exprCost.startup + exprCost.per_tuple;

    // Add scanning CPU cost
    QualCost qpqualCost;
    get_restriction_qual_cost(root, baseRelOptInfo, paramPathInfo, &qpqualCost);

    startup_cost += qpqualCost.startup;

    Cost run_cost = 0;

    Cost cpu_per_tuple = cpu_tuple_cost + qpqualCost.per_tuple;
    run_cost += cpu_per_tuple * baseRelOptInfo->tuples;
