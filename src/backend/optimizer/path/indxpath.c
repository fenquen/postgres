...
static void get_index_paths(PlannerInfo *root, RelOptInfo *relOptInfo,
                            IndexOptInfo *indexOptInfo, IndexClauseSet *indexClauseSet,
                            List **bitindexpaths);

static List *build_index_paths(PlannerInfo *root, RelOptInfo *relOptInfo,
                               IndexOptInfo *indexOptInfo, IndexClauseSet *indexClauseSet,
                               bool predicatable,
                               ScanTypeControl scanType,
                               bool *skipNonnativeSaop,
                               bool *skipLowerSaop);
...
static bool check_index_only(RelOptInfo *relOptInfo, IndexOptInfo *indexOptInfo);
 ...
 */
void create_index_paths(PlannerInfo *root, RelOptInfo *relOptInfo) {
    // no indexes
    if (relOptInfo->indexlist == NIL) {
        return;
    }

    /* Bitmap paths are collected and then dealt with at the end */
    List *bitJoinPathList = NIL;
    List *joinorClauseList = NIL;
    List *bitIndexPathList = NIL;

    IndexClauseSet restrictionClauseSet;
    IndexClauseSet joinClauseSet;
    IndexClauseSet equivalenceClauseSet;
    ...
    foreach(lc, relOptInfo->indexlist) {
        ...
        Assert(indexOptInfo->nkeycolumns <= INDEX_MAX_KEYS);
         ...
         */
        if (indexOptInfo->indpred != NIL && !indexOptInfo->predOK) {
            continue;
        }

        // identify the restriction clauses that can match the index.
        MemSet(&restrictionClauseSet, 0, sizeof(restrictionClauseSet));
        match_restriction_clauses_to_index(root, indexOptInfo, &restrictionClauseSet); // 搜罗where涉及到的index
         ...
         */
        get_index_paths(root,
                        relOptInfo,
                        indexOptInfo,
                        &restrictionClauseSet,
                        &bitIndexPathList);
         ...
         */
        MemSet(&joinClauseSet, 0, sizeof(joinClauseSet));
        match_join_clauses_to_index(root,
                                    relOptInfo,
                                    indexOptInfo,
                                    &joinClauseSet,
                                    &joinorClauseList);

        // look for EquivalenceClasses that can generate joinclauses matching the index.
        MemSet(&equivalenceClauseSet, 0, sizeof(equivalenceClauseSet));
        match_eclass_clauses_to_index(root, indexOptInfo, &equivalenceClauseSet);

        // if we found any plain or eclass join clauses, build parameterized index paths using them.
        if (joinClauseSet.nonempty || equivalenceClauseSet.nonempty) {
            consider_index_join_clauses(root,
                                        relOptInfo,
                                        indexOptInfo,
                                        &restrictionClauseSet,
                                        &joinClauseSet,
                                        &equivalenceClauseSet,
                                        &bitJoinPathList);
        }
    }

    // generate BitmapOrPaths for any suitable OR-clauses in restriction list. add these to bitindexpaths.
    List *indexPathList = generate_bitmap_or_paths(root,
                                                   relOptInfo,
                                                   relOptInfo->baserestrictinfo,
                                                   NIL);
    bitIndexPathList = list_concat(bitIndexPathList, indexPathList);

    // generate BitmapOrPaths for any suitable OR-clauses in join clause list. add these to bitjoinpaths.
    indexPathList = generate_bitmap_or_paths(root,
                                             relOptInfo,
                                             joinorClauseList,
                                             relOptInfo->baserestrictinfo);
    bitJoinPathList = list_concat(bitJoinPathList, indexPathList);
     ...
     */
    if (bitIndexPathList != NIL) {
        Path *bitmapqual;
        BitmapHeapPath *bpath;

        bitmapqual = choose_bitmap_and(root, relOptInfo, bitIndexPathList);
        bpath = create_bitmap_heap_path(root, relOptInfo, bitmapqual,
                                        relOptInfo->lateral_relids, 1.0, 0);
        add_path(relOptInfo, (Path *) bpath);

        /* create a partial bitmap heap path */
        if (relOptInfo->consider_parallel && relOptInfo->lateral_relids == NULL) {
            create_partial_bitmap_paths(root, relOptInfo, bitmapqual);
        }
    }
     ...
     * individual clauses.)
     */
    if (bitJoinPathList != NIL) {
        ListCell *listCell;

        /* Identify each distinct parameterization seen in bitjoinpaths */
        List *all_path_outers = NIL;
        foreach(listCell, bitJoinPathList) {
            Path *path = (Path *) lfirst(listCell);
        ...
        foreach(listCell, all_path_outers) {
            Relids max_outers = (Relids) lfirst(listCell);

            ListCell *lcp;

            // Identify all the bitmap join paths needing no more than that */
            List *this_path_set = NIL;
            foreach(lcp, bitJoinPathList) {
            ...
            // Add in restriction bitmap paths, since they can be used together with any join paths.
            this_path_set = list_concat(this_path_set, bitIndexPathList);

            /* Select best AND combination for this parameterization */
            Path *bitmapqual = choose_bitmap_and(root, relOptInfo, this_path_set);

            /* And push that path into the mix */
            Relids required_outer = PATH_REQ_OUTER(bitmapqual);
            double loop_count = get_loop_count(root, relOptInfo->relid, required_outer);

            BitmapHeapPath *bitmapHeapPath = create_bitmap_heap_path(root,
                                                                     relOptInfo,
                                                                     bitmapqual,
                                                                     required_outer,
                                                                     loop_count, 0);

            add_path(relOptInfo, (Path *) bitmapHeapPath);
        }
    }
}
...
static void get_index_paths(PlannerInfo *root,
     ...
     * paths if possible).
     */
    List *indexPathList = build_index_paths(root,
                                            relOptInfo,
                                            indexOptInfo,
                                            indexClauseSet,
                                            indexOptInfo->predOK,
     ...
     */
    if (skip_lower_saop) {
        indexPathList = list_concat(indexPathList,
                                    build_index_paths(root, relOptInfo,
                                                      indexOptInfo, indexClauseSet,
                                                      indexOptInfo->predOK,
                                                      ST_ANYSCAN,
                                                      &skip_nonnative_saop,
                                                      NULL));
     ...
     */
    foreach(lc, indexPathList) {
        IndexPath *indexPath = (IndexPath *) lfirst(lc);

        if (indexOptInfo->amhasgettuple)
            add_path(relOptInfo, (Path *) indexPath);

        if (indexOptInfo->amhasgetbitmap &&
            (indexPath->path.pathkeys == NIL ||
             indexPath->indexselectivity < 1.0))
            *bitindexpaths = lappend(*bitindexpaths, indexPath);
    }
    ...
    if (skip_nonnative_saop) {
        indexPathList = build_index_paths(root, relOptInfo,
                                          indexOptInfo, indexClauseSet,
                                          false,
                                          ST_BITMAPSCAN,
                                          NULL,
                                          NULL);
        *bitindexpaths = list_concat(*bitindexpaths, indexPathList);
    }
}
...
static List *build_index_paths(PlannerInfo *root,
    ...
    switch (scanType) {
        ...
        case ST_BITMAPSCAN:
            ...
            break;
        case ST_ANYSCAN: // either or both are all right
        default:
            break;
    }
     ...
     */
    List *indexClauseList = NIL;
    bool foundLowerSaopClause = false;

    Relids outerRelids = bms_copy(relOptInfo->lateral_relids);

    for (int indexcol = 0; indexcol < indexOptInfo->nkeycolumns; indexcol++) {
        ListCell *lc;
        foreach(lc, indexClauseSet->indexclauses[indexcol]) {
            IndexClause *iclause = (IndexClause *) lfirst(lc);
            RestrictInfo *restrictInfo = iclause->rinfo;

            /* We might need to omit ScalarArrayOpExpr clauses */
            if (IsA(restrictInfo->clause, ScalarArrayOpExpr)) {
                if (!indexOptInfo->amsearcharray) {
                    if (skipNonnativeSaop) {
                        /* Ignore because not supported by index */
                        *skipNonnativeSaop = true;
                        continue;
                    }
                ...
                if (indexcol > 0) {
                    ...
                    foundLowerSaopClause = true;
                }
            }

            /* OK to include this clause */
            indexClauseList = lappend(indexClauseList, iclause);
            outerRelids = bms_add_members(outerRelids, restrictInfo->clause_relids);
        }
         ...
         * clauses.)
         */
        if (indexClauseList == NIL && !indexOptInfo->amoptionalkey) {
            return NIL;
        }
    }

    // We do not want the index's rel itself listed in outer_relids
    outerRelids = bms_del_member(outerRelids, relOptInfo->relid);
    ...
    double loop_count = get_loop_count(root, relOptInfo->relid, outerRelids);
     ...
     */
    bool pathkeysPossiblyUseful = (scanType != ST_BITMAPSCAN &&
                                   !foundLowerSaopClause &&
                                   has_useful_pathkeys(root, relOptInfo));

    bool indexIsOrdered = (indexOptInfo->sortopfamily != NULL);

    List *indexPathKeyList;
    List *usefulIndexPathkeyList;
    List *orderByClauseList;
    List *orderByClauseColList;

    if (indexIsOrdered && pathkeysPossiblyUseful) {
        indexPathKeyList = build_index_pathkeys(root, indexOptInfo, ForwardScanDirection);
        usefulIndexPathkeyList = truncate_useless_pathkeys(root, relOptInfo, indexPathKeyList);
        orderByClauseList = NIL;
        orderByClauseColList = NIL;
    } else if (indexOptInfo->amcanorderbyop && pathkeysPossiblyUseful) {
        /* see if we can generate ordering operators for query_pathkeys */
        match_pathkeys_to_index(indexOptInfo,
                                root->query_pathkeys,
                                &orderByClauseList,
                                &orderByClauseColList);

        if (orderByClauseList) {
            usefulIndexPathkeyList = root->query_pathkeys;
        } else {
            usefulIndexPathkeyList = NIL;
        }
    } else {
        usefulIndexPathkeyList = NIL;
        orderByClauseList = NIL;
     ...
     */
    bool indexOnlyScan = (scanType != ST_BITMAPSCAN && check_index_only(relOptInfo, indexOptInfo));
    ...
    if (indexClauseList != NIL || usefulIndexPathkeyList != NIL || predicatable || indexOnlyScan) {

        IndexPath *indexPath = create_index_path(root,
                                                 indexOptInfo,
                                                 indexClauseList,
                                                 orderByClauseList,
                                                 orderByClauseColList,
                                                 usefulIndexPathkeyList,
                                                 indexIsOrdered ? ForwardScanDirection : NoMovementScanDirection,
                                                 indexOnlyScan,
                                                 outerRelids,
                                                 loop_count,
                                                 false);
        indexPathList = lappend(indexPathList, indexPath);

        // if appropriate, consider parallel index scan,don't allow parallel index scan for bitmap index scans.
        if (indexOptInfo->amcanparallel &&
            relOptInfo->consider_parallel &&
            outerRelids == NULL &&
            scanType != ST_BITMAPSCAN) {

            IndexPath *indexPath1 = create_index_path(root,
                                                      indexOptInfo,
                                                      indexClauseList,
                                                      orderByClauseList,
                                                      orderByClauseColList,
            ...
            // if after costing the path, we find that it's not worth using parallel workers, just free it.
            if (indexPath1->path.parallel_workers > 0) {
                add_partial_path(relOptInfo, (Path *) indexPath1);
            } else {
                pfree(indexPath1);
            }
        }
    ...
    if (indexIsOrdered && pathkeysPossiblyUseful) {
        indexPathKeyList = build_index_pathkeys(root, indexOptInfo, BackwardScanDirection);
        usefulIndexPathkeyList = truncate_useless_pathkeys(root, relOptInfo, indexPathKeyList);

        if (usefulIndexPathkeyList != NIL) {
            IndexPath *indexPath = create_index_path(root, indexOptInfo,
                                                     indexClauseList,
                                                     NIL,
                                                     NIL,
                                                     usefulIndexPathkeyList,
            ...
            if (indexOptInfo->amcanparallel &&
                ...
                indexPath = create_index_path(root, indexOptInfo,
                                              indexClauseList,
                                              NIL,
                                              NIL,
                                              usefulIndexPathkeyList,
                ...
                // if after costing the path, we find that it's not worth using parallel workers, just free it
                if (indexPath->path.parallel_workers > 0) {
                    add_partial_path(relOptInfo, (Path *) indexPath);
                } else {
                    pfree(indexPath);
                }
            }
...
static bool check_index_only(RelOptInfo *relOptInfo, IndexOptInfo *indexOptInfo) {
    ...
    // check that all needed attributes of the relation are available from the index.
     ...
     */
    pull_varattnos((Node *) relOptInfo->reltarget->exprs, relOptInfo->relid, &attrs_used);
     ...
     */
    ListCell *lc;
    foreach(lc, indexOptInfo->indrestrictinfo) {
        RestrictInfo *restrictInfo = (RestrictInfo *) lfirst(lc);

        pull_varattnos((Node *) restrictInfo->clause, relOptInfo->relid, &attrs_used);
    }
     ...
     */
    for (int i = 0; i < indexOptInfo->ncolumns; i++) {
        int attno = indexOptInfo->indexkeys[i];
...
static void match_clauses_to_index(PlannerInfo *root,
                                   ...
                                   IndexClauseSet *indexClauseSet) {
    ListCell *listCell;
    foreach(listCell, clauseList) {
        RestrictInfo *restrictInfo = lfirst_node(RestrictInfo, listCell);
...
static void match_clause_to_index(PlannerInfo *root,
     ...
     */
    if (!restriction_is_securely_promotable(restrictInfo, indexOptInfo->rel)) {
        return;
    }

    /* OK, check each index 对应的 column for a match */
    for (int keyColIndexInIndex = 0; keyColIndexInIndex < indexOptInfo->nkeycolumns; keyColIndexInIndex++) {
        // ignore duplicate
        ListCell *listCell;
        foreach(listCell, indexClauseSet->indexclauses[keyColIndexInIndex]) {
            IndexClause *indexClause = (IndexClause *) lfirst(listCell);
            if (indexClause->rinfo == restrictInfo) {
                return;
            }
        }

        /* OK, try to match the clause to the index column */
        IndexClause *indexClause = match_clause_to_indexcol(root,
                                                            restrictInfo,
                                                            keyColIndexInIndex,
                                                            indexOptInfo);
        // success so record it
        if (indexClause) {
            indexClauseSet->indexclauses[keyColIndexInIndex] =
                    lappend(indexClauseSet->indexclauses[keyColIndexInIndex], indexClause);
            indexClauseSet->nonempty = true;
            return;
        }
    }
}
...
static IndexClause *match_clause_to_indexcol(PlannerInfo *root,
    ...
    Expr *restrictInfoClause = restrictInfo->clause;

    Assert(keyColIndexInIndex < indexOptInfo->nkeycolumns);
     ...
     */
    if (restrictInfoClause == NULL) {
        return NULL;
    }

    /* First check for boolean-index cases. */
    Oid opfamily = indexOptInfo->opfamily[keyColIndexInIndex]; // 该索引的这个字段对应的 INTEGER_BTREE_FAM_OID 1976
    if (IsBooleanOpfamily(opfamily)) {
        IndexClause *iclause = match_boolean_index_clause(root, restrictInfo, keyColIndexInIndex, indexOptInfo);
        if (iclause) {
            return iclause;
        }
    }
     ...
     */
    if (IsA(restrictInfoClause, OpExpr)) {
        return match_opclause_to_indexcol(root, restrictInfo, keyColIndexInIndex, indexOptInfo);
    }

    if (IsA(restrictInfoClause, FuncExpr)) {
        return match_funcclause_to_indexcol(root, restrictInfo, keyColIndexInIndex, indexOptInfo);
    }

    if (IsA(restrictInfoClause, ScalarArrayOpExpr)) {
        return match_saopclause_to_indexcol(root, restrictInfo, keyColIndexInIndex, indexOptInfo);
    }

    if (IsA(restrictInfoClause, RowCompareExpr)) {
        return match_rowcompare_to_indexcol(root, restrictInfo, keyColIndexInIndex, indexOptInfo);
    }

    if (indexOptInfo->amsearchnulls && IsA(restrictInfoClause, NullTest)) {
        NullTest *nt = (NullTest *) restrictInfoClause;

        if (!nt->argisrow &&
            match_index_to_operand((Node *) nt->arg, keyColIndexInIndex, indexOptInfo)) {
            IndexClause *iclause = makeNode(IndexClause);
            iclause->rinfo = restrictInfo;
            iclause->indexquals = list_make1(restrictInfo);
            iclause->lossy = false;
            iclause->indexcol = keyColIndexInIndex;
...
static IndexClause *match_opclause_to_indexcol(PlannerInfo *root,
                                               ...
                                               int keyColIndexInIndex,
                                               IndexOptInfo *indexOptInfo) {
    OpExpr *restrictExpr = (OpExpr *) restrictInfo->clause;
     ...
     */
    if (list_length(restrictExpr->args) != 2) {
        return NULL;
    }

    Node *leftOperand = (Node *) linitial(restrictExpr->args);
    Node *rightOperand = (Node *) lsecond(restrictExpr->args);

    Oid restrictExprOid = restrictExpr->opno;
    Oid restrictExprCollationOid = restrictExpr->inputcollid;

    Index index_relid = indexOptInfo->rel->relid;// 对应的table在sql中的位置(1based)
    Oid indexCurKeyColOpFamilyOid = indexOptInfo->opfamily[keyColIndexInIndex];
    Oid indexCurKeyColCollationOid = indexOptInfo->indexcollations[keyColIndexInIndex];
     ...
     */
    if (match_index_to_operand(leftOperand, keyColIndexInIndex, indexOptInfo) &&
        !bms_is_member(index_relid, restrictInfo->right_relids) &&
        !contain_volatile_functions(rightOperand)) {

        if (IndexCollMatchesExprColl(indexCurKeyColCollationOid, restrictExprCollationOid) &&
            op_in_opfamily(restrictExprOid, indexCurKeyColOpFamilyOid)) {

            IndexClause *indexClause = makeNode(IndexClause);
            indexClause->rinfo = restrictInfo;
            indexClause->indexquals = list_make1(restrictInfo);
            indexClause->lossy = false;
            indexClause->indexcol = keyColIndexInIndex;
         ...
         */
        set_opfuncid(restrictExpr);    /* make sure we have opfuncid */

        return get_index_clause_from_support(root,
                                             restrictInfo,
                                             restrictExpr->opfuncid,
                                             0, /* indexarg on left */
                                             keyColIndexInIndex,
                                             indexOptInfo);
    }

    if (match_index_to_operand(rightOperand, keyColIndexInIndex, indexOptInfo) &&
        !bms_is_member(index_relid, restrictInfo->left_relids) &&
        !contain_volatile_functions(leftOperand)) {

        if (IndexCollMatchesExprColl(indexCurKeyColCollationOid, restrictExprCollationOid)) {
            Oid comm_op = get_commutator(restrictExprOid);

            if (OidIsValid(comm_op) && op_in_opfamily(comm_op, indexCurKeyColOpFamilyOid)) {

                /* Build a commuted OpExpr and RestrictInfo */
                RestrictInfo *commrinfo = commute_restrictinfo(restrictInfo, comm_op);

                /* Make an IndexClause showing that as a derived qual */
                IndexClause *indexClause = makeNode(IndexClause);
                indexClause->rinfo = restrictInfo;
                indexClause->indexquals = list_make1(commrinfo);
                indexClause->lossy = false;
                indexClause->indexcol = keyColIndexInIndex;
                indexClause->indexcols = NIL;
                return indexClause;
...
check_index_predicates(PlannerInfo *root, RelOptInfo *rel) {
    ...
    foreach(lc, rel->indexlist) {
        ...
        if (indexOptInfo->indpred == NIL)
            continue;            /* ignore non-partial indexes here */

        /* don't repeat work if already proven OK */
        if (!indexOptInfo->predOK) {
            indexOptInfo->predOK = predicate_implied_by(indexOptInfo->indpred, clauselist, false);
        }
        ...
        indexOptInfo->indrestrictinfo = NIL;
        foreach(lcr, rel->baserestrictinfo) {
            RestrictInfo *rinfo = (RestrictInfo *) lfirst(lcr);

            /* predicate_implied_by() assumes first arg is immutable */
            if (contain_mutable_functions((Node *) rinfo->clause) ||
                !predicate_implied_by(list_make1(rinfo->clause), indexOptInfo->indpred, false))
                indexOptInfo->indrestrictinfo = lappend(indexOptInfo->indrestrictinfo, rinfo);
        }
    }
}
...
bool match_index_to_operand(Node *operand,
    ...
    int indkey = indexOptInfo->indexkeys[keyColIndexInIndex];
    if (indkey != 0) {
        // simple index column; operand must be a matching Var.
        if (operand && IsA(operand, Var) &&
            indexOptInfo->rel->relid == ((Var *) operand)->varno && // 对应的表的位置对的上
            indkey == ((Var *) operand)->varattno) { // 对应的column在表的位置也对的上
            return true;
        }
    } else {
        /*
        ...
        indexpr_item = list_head(indexOptInfo->indexprs);
        for (i = 0; i < keyColIndexInIndex; i++) {
            if (indexOptInfo->indexkeys[i] == 0) {
