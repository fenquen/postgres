...
static void hash_inner_and_outer(PlannerInfo *root,
                                 ...
                                 JoinType joinType, JoinPathExtraData *joinPathExtraData);
...
void add_paths_to_joinrel(PlannerInfo *root,
    ...
    if (joinRelOptInfo->reloptkind == RELOPT_OTHER_JOINREL) {
        ...
        joinrelids = joinRelOptInfo->relids;
    }
...
static void try_hashjoin_path(PlannerInfo *root,
     ...
     * parameterization wouldn't be sensible.
     */
    Relids requiredOuter = calc_non_nestloop_required_outer(outerPath,
                                                            innerPath);
    if (requiredOuter &&
        !bms_overlap(requiredOuter, joinPathExtraData->param_source_rels)) {
    ...
    if (add_path_precheck(joinRelOptInfo,
                          joinCostWorkspace.startup_cost,
                          joinCostWorkspace.total_cost,
                          NIL,
                          requiredOuter)) {
        HashPath *hashPath = create_hashjoin_path(root,
                                                  joinRelOptInfo,
                                                  joinType,
                                                  &joinCostWorkspace,
                                                  joinPathExtraData,
                                                  ...
                                                  innerPath,
                                                  false,    // parallel_hash
                                                  joinPathExtraData->restrictlist,
                                                  requiredOuter,
                                                  hashClauseList);
        add_path(joinRelOptInfo, (Path *) hashPath);
    } else { // waste no memory when we reject a path here */
        bms_free(requiredOuter);
    }
}
...
static void hash_inner_and_outer(PlannerInfo *root,
     ...
     */
    List *hashClauseList = NIL;
    ListCell *listCell;
    foreach(listCell, joinPathExtraData->restrictlist) {
         ...
         */
        if (isOuterJoin && RINFO_IS_PUSHED_DOWN(restrictInfo, joinRelOptInfo->relids)) {
            continue;
        }
        ...
        if (!clause_sides_match_join(restrictInfo, outerRelOptInfo, innerRelOptInfo)) {
            continue; // no good for these input relations
        }

        hashClauseList = lappend(hashClauseList, restrictInfo);
    }

    // If we found any usable hash clause, make paths
    if (hashClauseList) {
        /*
         ...
         */
        Path *outerCheapestStartupPath = outerRelOptInfo->cheapest_startup_path;
        Path *outerCheapestTotalPath = outerRelOptInfo->cheapest_total_path;
        Path *innerCheapestTotalPath = innerRelOptInfo->cheapest_total_path;
         ...
         * available paths.)
         */
        if (PATH_PARAM_BY_REL(outerCheapestTotalPath, innerRelOptInfo) ||
            PATH_PARAM_BY_REL(innerCheapestTotalPath, outerRelOptInfo)) {
            return;
        }
        ...
        switch (joinType) {
            case JOIN_UNIQUE_OUTER: {
                outerCheapestTotalPath = (Path *) create_unique_path(root,
                                                                     outerRelOptInfo,
                                                                     outerCheapestTotalPath,
                                                                     joinPathExtraData->sjinfo);
                Assert(outerCheapestTotalPath);
                ...
                break;
            case JOIN_UNIQUE_INNER: {
                innerCheapestTotalPath = (Path *) create_unique_path(root,
                                                                     innerRelOptInfo,
                                                                     innerCheapestTotalPath,
                                                                     joinPathExtraData->sjinfo);
                Assert(innerCheapestTotalPath);
                joinType = JOIN_INNER;
                try_hashjoin_path(root,
                                  joinRelOptInfo,
                                  outerCheapestTotalPath,
                ...
                if (outerCheapestStartupPath != NULL &&
                    outerCheapestStartupPath != outerCheapestTotalPath)
                    try_hashjoin_path(root,
                                      joinRelOptInfo,
                                      outerCheapestStartupPath,
                                      innerCheapestTotalPath,
                                      hashClauseList,
                                      joinType,
                                      joinPathExtraData);
            ...
            default: {
                ...
                ListCell *lc1;
                foreach(lc1, outerRelOptInfo->cheapest_parameterized_paths) {
                    Path *outerpath = (Path *) lfirst(lc1);

                    // We cannot use an outer path that is parameterized by the inner rel.
                    if (PATH_PARAM_BY_REL(outerpath, innerRelOptInfo)) {
                        continue;
                    }

                    ListCell *lc2;
                    foreach(lc2, innerRelOptInfo->cheapest_parameterized_paths) {
                        Path *innerpath = (Path *) lfirst(lc2);

                        // we cannot use an inner path that is parameterized by the outer rel, either.
                        if (PATH_PARAM_BY_REL(innerpath, outerRelOptInfo))
                            continue;

                        /* already tried it */
                        if (outerpath == outerCheapestStartupPath && innerpath == innerCheapestTotalPath)
                            continue;
         ...
         */
        if (joinRelOptInfo->consider_parallel &&
            save_jointype != JOIN_UNIQUE_OUTER &&
            save_jointype != JOIN_FULL &&
            save_jointype != JOIN_RIGHT &&
            outerRelOptInfo->partial_pathlist != NIL &&
            bms_is_empty(joinRelOptInfo->lateral_relids)) {
            Path *cheapest_partial_outer;
            Path *cheapest_partial_inner = NULL;
            Path *cheapest_safe_inner = NULL;

            cheapest_partial_outer =
                    (Path *) linitial(outerRelOptInfo->partial_pathlist);
             ...
             */
            if (innerRelOptInfo->partial_pathlist != NIL &&
                save_jointype != JOIN_UNIQUE_INNER &&
                enable_parallel_hash) {
                cheapest_partial_inner =
                        (Path *) linitial(innerRelOptInfo->partial_pathlist);
                try_partial_hashjoin_path(root, joinRelOptInfo,
                                          cheapest_partial_outer,
                                          cheapest_partial_inner,
                                          hashClauseList, joinType, joinPathExtraData,
                                          true /* parallel_hash */ );
            }
             ...
             * inner path.
             */
            if (innerCheapestTotalPath->parallel_safe)
                cheapest_safe_inner = innerCheapestTotalPath;
            else if (save_jointype != JOIN_UNIQUE_INNER)
                cheapest_safe_inner =
                        get_cheapest_parallel_safe_total_inner(innerRelOptInfo->pathlist);
