...
static void populate_joinrel_with_paths(PlannerInfo *root, RelOptInfo *outerRelOptInfo,
                                        RelOptInfo *innerRelOptInfo, RelOptInfo *joinRelOptInfo,
                                        SpecialJoinInfo *specialJoinInfo, List *restrictList);
...
void join_search_one_level(PlannerInfo *root, int level) {
     ...
     */
    ListCell *listCell;
    foreach(listCell, joinrels[level - 1]) {
        RelOptInfo *old_rel = (RelOptInfo *) lfirst(listCell);
    ...
    for (int a = 2;; a++) {
        ...
        foreach(listCell, joinrels[a]) {
            ...
            if (a == other_level)
                other_rels = lnext(listCell);    /* only consider remaining rels */
            else
...
static void make_rels_by_clause_joins(PlannerInfo *root,
    ...
    for_each_cell(l, other_rels) {
        RelOptInfo *otherRelOptInfo = (RelOptInfo *) lfirst(l);

        if (!bms_overlap(oldRelOptInfo->relids, otherRelOptInfo->relids) &&
            (have_relevant_joinclause(root, oldRelOptInfo, otherRelOptInfo) ||
             have_join_order_restriction(root, oldRelOptInfo, otherRelOptInfo))) {
            (void) make_join_rel(root, oldRelOptInfo, otherRelOptInfo);
        }
    }
}
...
RelOptInfo *make_join_rel(PlannerInfo *root,
    ...
    Assert(!bms_overlap(outerRelOptInfo->relids, innerRelOptInfo->relids));

    /* Construct Relids set that identifies the joinrel. */
    Relids joinRelids = bms_union(outerRelOptInfo->relids, innerRelOptInfo->relids);
    ...
    if (reversed) {
        RelOptInfo *trel = outerRelOptInfo;
        outerRelOptInfo = innerRelOptInfo;
        innerRelOptInfo = trel;
    }
     ...
     */
    if (specialJoinInfo == NULL) {
        specialJoinInfo = &sjinfo_data;
        specialJoinInfo->type = T_SpecialJoinInfo;
        specialJoinInfo->min_lefthand = outerRelOptInfo->relids;
        specialJoinInfo->min_righthand = innerRelOptInfo->relids;
        specialJoinInfo->syn_lefthand = outerRelOptInfo->relids;
        specialJoinInfo->syn_righthand = innerRelOptInfo->relids;
        specialJoinInfo->jointype = JOIN_INNER;
        /* we don't bother trying to make the remaining fields valid */
        specialJoinInfo->lhs_strict = false;
        specialJoinInfo->delay_upper_joins = false;
        specialJoinInfo->semi_can_btree = false;
        specialJoinInfo->semi_can_hash = false;
        specialJoinInfo->semi_operators = NIL;
        specialJoinInfo->semi_rhs_exprs = NIL;
    }

    // find or build the join RelOptInfo, and compute the restrictlist that goes with this particular joining.
    List *restrictList;
    RelOptInfo *joinRelOptInfo = build_join_rel(root,
                                                joinRelids,
                                                outerRelOptInfo,
                                                innerRelOptInfo,
                                                specialJoinInfo,
    ...
    // if we've already proven this join is empty, we needn't consider any more paths for it.
    if (is_dummy_rel(joinRelOptInfo)) {
        bms_free(joinRelids);
        return joinRelOptInfo;
    }
