 ...
 */
void add_base_rels_to_query(PlannerInfo *root, Node *from) {
    if (from == NULL) {
        return;
    }

    if (IsA(from, RangeTblRef)) {
        build_simple_rel(root, ((RangeTblRef *) from)->rtindex, NULL);
    } else if (IsA(from, FromExpr)) { // fromExpr 的 fromlist 还是会包含多个原子的 rangeTblRef 需要递归
        FromExpr *fromExpr = (FromExpr *) from;
        ListCell *listCell;

        foreach(listCell, fromExpr->fromlist) {
            add_base_rels_to_query(root, lfirst(listCell));
        }
    } else if (IsA(from, JoinExpr)) {
        JoinExpr *joinExpr = (JoinExpr *) from;

        add_base_rels_to_query(root, joinExpr->larg);
        add_base_rels_to_query(root, joinExpr->rarg);
    } else {
...
List *deconstruct_jointree(PlannerInfo *root) {
    ...
    result = deconstruct_recurse(root,
