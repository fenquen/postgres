...
RelOptInfo *query_planner(PlannerInfo *root,
    ...
    RelOptInfo *finalRelOptInfo;

    // Init planner lists to empty.
    // NOTE: append_rel_list was set up by subquery_planner, so do not touch here.
     ...
     * OK because the rangetable won't change any more), and set up an empty array for indexing base relations.
     */
    ...
    if (list_length(parse->jointree->fromlist) == 1) {
        ...
        if (IsA(jtnode, RangeTblRef)) {
            ...
            if (rte->rtekind == RTE_RESULT) {
                ...
                finalRelOptInfo = build_simple_rel(root, varno, NULL);
                ...
                if (root->glob->parallelModeOK &&
                    ...
                    finalRelOptInfo->consider_parallel =
                ...
                add_path(finalRelOptInfo, (Path *)
                        create_group_result_path(root, finalRelOptInfo,
                                                 finalRelOptInfo->reltarget,
                                                 (List *) parse->jointree->quals));

                /* Select cheapest path (pretty easy in this case...) */
                set_cheapest(finalRelOptInfo);
                ...
                return finalRelOptInfo;
            }
        }
    }

    // populate append_rel_array with each AppendRelInfo to allow direct lookups by child relid.
    setup_append_rel_array(root);

    /*
     * 会生成relOptInfo 注入到了 root的 simple_rte_array
    ...
    // clauses as appropriate.  (Implied join clauses are formed on-the-fly later.)
     ...
     * done after join removal because removal could change whether a placeholder is evaluable at a base rel.
     */
    add_placeholders_to_base_rels(root);

    // construct the lateral reference sets now that we have finalized PlaceHolderVar eval levels.
    ...
    // look for join OR clauses that we can extract single-relation restriction OR clauses from.
    ...
    // pg中貌似逻辑和物理优化的边界很不清晰度
    finalRelOptInfo = make_one_rel(root, joinlist);

    /* Check that we got at least one usable path */
    if (!finalRelOptInfo || !finalRelOptInfo->cheapest_total_path ||
        finalRelOptInfo->cheapest_total_path->param_info != NULL)
