 ...
 *
 *****************************************************************************/
PlannedStmt *planner(Query *query, int cursorOptions, ParamListInfo boundParams) {
...
PlannedStmt *standard_planner(Query *query,
                              int cursorOptions,
	 ...
	 * PlannerInfo.
	 */
    PlannerGlobal *plannerGlobal = makeNode(PlannerGlobal);
    ...
    if ((cursorOptions & CURSOR_OPT_PARALLEL_OK) != 0 &&
        ...
        plannerGlobal->maxParallelHazard = max_parallel_hazard(query);
    ...
    if (cursorOptions & CURSOR_OPT_FAST_PLAN) {
		 ...
		 * process some of the tuples sooner.  Use a GUC parameter to decide what fraction to optimize for.
		 */
    ...
    // optionally add a Gather node for testing purposes, provided this is actually a safe thing to do.
    if (force_parallel_mode != FORCE_PARALLEL_OFF && topPlan->parallel_safe) {
		 ...
		 * move them up to the Gather node; same as we do for Material node in materialize_finished_plan.
		 */
        ...
        // since this Gather has no parallel-aware descendants to signal to,we don't need a rescan Param.
		 ...
		 */
        gather->plan.startup_cost = topPlan->startup_cost + parallel_setup_cost;
        gather->plan.total_cost = topPlan->total_cost + parallel_setup_cost + parallel_tuple_cost * topPlan->plan_rows;
    ...
    plannedStmt->hasReturning = (query->returningList != NIL);
    plannedStmt->hasModifyingCTE = query->hasModifyingCTE;
...
/*--------------------
 * 迭代的子查询优化
 * invokes the planner on subquery.  We recurse here for each sub-SELECT in the query tree.
 *
 * plannerGlobal is the global state for the current planner run.
 ...
 *--------------------
 */
PlannerInfo *subquery_planner(PlannerGlobal *plannerGlobal,
                              Query *query,
    ...
    root->parse = query;
    root->glob = plannerGlobal;
    root->query_level = parentRoot ? parentRoot->query_level + 1 : 1;
    root->parent_root = parentRoot;
	 ...
	 */
    root->hasHavingQual = (query->havingQual != NULL);
    ...
    if (query->hasTargetSRFs)
        query->hasTargetSRFs = expression_returns_set((Node *) query->targetList);

    newWithCheckOptions = NIL;
    foreach(l, query->withCheckOptions) {
    ...
    query->withCheckOptions = newWithCheckOptions;

    query->returningList = (List *)
            preprocess_expression(root, (Node *) query->returningList,
                                  EXPRKIND_TARGET);

    preprocess_qual_conditions(root, (Node *) query->jointree);

    query->havingQual = preprocess_expression(root, query->havingQual,
                                              EXPRKIND_QUAL);
    ...
    query->limitOffset = preprocess_expression(root, query->limitOffset,
                                               EXPRKIND_LIMIT);
    query->limitCount = preprocess_expression(root, query->limitCount,
                                              EXPRKIND_LIMIT);

    if (query->onConflict) {
        query->onConflict->arbiterElems = (List *)
                preprocess_expression(root,
                                      (Node *) query->onConflict->arbiterElems,
                                      EXPRKIND_ARBITER_ELEM);
        query->onConflict->arbiterWhere =
                preprocess_expression(root,
                                      query->onConflict->arbiterWhere,
                                      EXPRKIND_QUAL);
        query->onConflict->onConflictSet = (List *)
                preprocess_expression(root,
                                      (Node *) query->onConflict->onConflictSet,
                                      EXPRKIND_TARGET);
        query->onConflict->onConflictWhere =
                preprocess_expression(root,
                                      query->onConflict->onConflictWhere,
                                      EXPRKIND_QUAL);
    ...
    foreach(l, (List *) query->havingQual) {
        ...
        } else if (query->groupClause && !query->groupingSets) {
            /* move it to WHERE */
            query->jointree->quals = (Node *)
                    lappend((List *) query->jointree->quals, havingclause);
        } else {
            /* put a copy in WHERE, keep it in HAVING */
            query->jointree->quals = (Node *)
                    lappend((List *) query->jointree->quals,
                            copyObject(havingclause));
            newHaving = lappend(newHaving, havingclause);
        }
    }
    query->havingQual = (Node *) newHaving;
    ...
    if (query->resultRelation &&
        rt_fetch(query->resultRelation, query->rtable)->inh)
...
preprocess_qual_conditions(PlannerInfo *root, Node *jtnode) {
    ...
    if (IsA(jtnode, RangeTblRef)) {
        ...
        foreach(l, f->fromlist) preprocess_qual_conditions(root, lfirst(l));
 ...
 *	  0 < tuple_fraction < 1: expect the given fraction of tuples available from the plan to be retrieved
...
static void grouping_planner(PlannerInfo *root,
    ...
    RelOptInfo *currentRelOptInfo;

    FinalPathExtraData extra;
    ListCell *lc;

    /* Tweak caller-supplied tuple_fraction if have LIMIT/OFFSET */
    int64 limitOffset_est = 0;
    int64 limitCount_est = 0;
    if (query->limitCount || query->limitOffset) {
        tuple_fraction = preprocess_limit(root, tuple_fraction, &limitOffset_est, &limitCount_est);

        /*
		 * If we have a known LIMIT, and don't have an unknown OFFSET, we can
		 * estimate the effects of using a bounded sort.
		 */
        if (limitCount_est > 0 && limitOffset_est >= 0) {
            limit_tuples = (double) limitCount_est + (double) limitOffset_est;
        }
    }
    ...
    if (query->setOperations) {
		 ...
		 * plan_set_operations.
		 */
        currentRelOptInfo = plan_set_operations(root);
        ...
        final_target = currentRelOptInfo->cheapest_total_path->pathtarget;
			  ...
			  translator: %s is a SQL row locking clause such as FOR UPDATE */
                            errmsg("%s is not allowed with UNION/INTERSECT/EXCEPT",
                                   LCS_asString(linitial_node(RowMarkClause,
                                                              query->rowMarks)->strength))));

        /*
		 * Calculate pathkeys that represent result ordering requirements
		 */
        Assert(query->distinctClause == NIL);
        ...
        if (query->groupingSets) {
            ...
            if (query->groupClause)
                query->groupClause = preprocess_groupclause(root, NIL);
        }
        ...
        if (query->hasAggs) {
            ...
            get_agg_clause_costs(root, query->havingQual, AGGSPLIT_SIMPLE,
                                 &agg_costs);
        }
        ...
        if (query->hasWindowFuncs) {
            wflists = find_window_functions((Node *) root->processed_tlist,
                                            list_length(query->windowClause));
		 ...
		 */
        currentRelOptInfo = query_planner(root, standard_qp_callback, &qp_extra);
		 ...
		 * plan node must emit what the grouping step wants; otherwise, it should emit grouping_target.
		 */
        have_grouping = (query->groupClause || query->groupingSets || query->hasAggs || root->hasHavingQual);
        if (have_grouping) {
        ...
        if (query->hasTargetSRFs) {
            ...
            split_pathtarget_at_srfs(root, sort_input_target,
                                     grouping_target,
                                     &sort_input_targets,
        ...
        scanjoin_target_same_exprs = list_length(scanjoin_targets) == 1
                                     && equal(scanjoin_target->exprs, currentRelOptInfo->reltarget->exprs);
		 ...
		 */
        if (have_grouping) {
            currentRelOptInfo = create_grouping_paths(root,
                                                      currentRelOptInfo,
                                                      grouping_target,
            ...
            if (query->hasTargetSRFs) {
                adjust_paths_for_srfs(root, currentRelOptInfo,
                                      grouping_targets,
                                      grouping_targets_contain_srfs);
            }
        }
		 ...
		 */
        if (activeWindows) {
            currentRelOptInfo = create_window_paths(root,
                                                    currentRelOptInfo,
                                                    grouping_target,
                                                    sort_input_target,
        ...
        if (query->distinctClause) {
            currentRelOptInfo = create_distinct_paths(root,
                                                      currentRelOptInfo);
        }
	 ...
	 */
    if (query->sortClause) {
        currentRelOptInfo = create_ordered_paths(root,
                                                 currentRelOptInfo,
                                                 final_target,
                                                 final_target_parallel_safe,
	 ...
	 * not a SELECT, consider_parallel will be false for every relation in the query.
	 */
	 ...
	 */
    foreach(lc, currentRelOptInfo->pathlist) {
		 ...
		 */
        if (query->commandType != CMD_SELECT && !inheritance_update) {
            Index rootRelation;
			 ...
			 */
            if (rt_fetch(query->resultRelation, query->rtable)->relkind ==
                RELKIND_PARTITIONED_TABLE)
                rootRelation = query->resultRelation;
            else
                rootRelation = 0;

            // Set up the WITH CHECK OPTION and RETURNING lists-of-lists, if needed.
            if (query->withCheckOptions)
                withCheckOptionLists = list_make1(query->withCheckOptions);
            else
                withCheckOptionLists = NIL;

            if (query->returningList)
                returningLists = list_make1(query->returningList);
            else
                returningLists = NIL;
            ...
            path = (Path *) create_modifytable_path(root, finalRelOptInfo,
                                                    ...
                                                    rootRelation,
                                                    false,
                                                    list_make1_int(query->resultRelation),
                                                    list_make1(path),
                                                    list_make1(root),
                                                    withCheckOptionLists,
                                                    returningLists,
    ...
    extra.limit_needed = limit_needed(query);
    extra.limit_tuples = limit_tuples;
    extra.count_est = limitCount_est;
 ...
 */
Path *get_cheapest_fractional_path(RelOptInfo *relOptInfo, double tuple_fraction) {
    Path *best_path = relOptInfo->cheapest_total_path;
    ...
    ListCell *listCell;
    foreach(listCell, relOptInfo->pathlist) {
        Path *path = (Path *) lfirst(listCell);

        if (path == relOptInfo->cheapest_total_path ||
