...
int compare_path_costs(Path *path1, Path *path2, CostSelector criterion) {
    if (criterion == STARTUP_COST) {
        ...
        // If paths have the same startup cost (not at all unlikely), order them by total cost.
 ...
 */
void set_cheapest(RelOptInfo *parentRelOptInfo) {
    ...
    Assert(IsA(parentRelOptInfo, RelOptInfo));

    if (parentRelOptInfo->pathlist == NIL) {
    ...
    foreach(p, parentRelOptInfo->pathlist) {
    ...
    parentRelOptInfo->cheapest_startup_path = cheapest_startup_path;
    parentRelOptInfo->cheapest_total_path = cheapest_total_path;
    parentRelOptInfo->cheapest_unique_path = NULL;    /* computed only if needed */
    parentRelOptInfo->cheapest_parameterized_paths = parameterized_paths;
}

/*
 * add_path 添加到 relOptinfo的pathlist
 ...
 */
void add_path(RelOptInfo *parentRelOptInfo, Path *new_path) {
    bool acceptNew = true;    /* unless we find a superior old path */
    ...
    ListCell *p1_next;
    for (ListCell *p1 = list_head(parentRelOptInfo->pathlist); p1 != NULL; p1 = p1_next) {
         ...
         * though it's cheap to compare there's not much point in checking it earlier.)
         */
                    ...
                    case COSTS_EQUAL:
                        ...
                        if (keyscmp == PATHKEYS_BETTER1) {
                            ...
                            if ((outercmp == BMS_EQUAL ||
                                ...
                                new_path->rows >= old_path->rows &&
                                new_path->parallel_safe <= old_path->parallel_safe)
                                acceptNew = false; /* old dominates new */
         ...
         */
        if (remove_old) {
            parentRelOptInfo->pathlist = list_delete_cell(parentRelOptInfo->pathlist, p1, p1_prev);
    ...
    if (acceptNew) {
        ...
        if (insert_after) {
            lappend_cell(parentRelOptInfo->pathlist, insert_after, new_path);
        } else {
            // 把path添加到parentRelOptInfo->pathlist打头
            parentRelOptInfo->pathlist = lcons(new_path, parentRelOptInfo->pathlist);
        }
    } else { // reject and recycle the new path
...
Path *create_functionscan_path(PlannerInfo *root,
    ...
    cost_functionscan(path, root, relOptInfo, path->param_info);
 ...
 */
LimitPath *
create_limit_path(PlannerInfo *root,
                  RelOptInfo *rel,
                  Path *subpath,
                  Node *limitOffset,
                  Node *limitCount,
                  int64 offset_est,
                  int64 count_est) {
    LimitPath *limitPath = makeNode(LimitPath);

    limitPath->path.pathtype = T_Limit;
    limitPath->path.parent = rel;
    /* Limit doesn't project, so use source path's pathtarget */
    limitPath->path.pathtarget = subpath->pathtarget;
    /* For now, assume we are above any joins, so no parameterization */
    limitPath->path.param_info = NULL;
    limitPath->path.parallel_aware = false;
    limitPath->path.parallel_safe = rel->consider_parallel && subpath->parallel_safe;
    limitPath->path.parallel_workers = subpath->parallel_workers;
    limitPath->path.rows = subpath->rows;
    limitPath->path.startup_cost = subpath->startup_cost;
    limitPath->path.total_cost = subpath->total_cost;
    limitPath->path.pathkeys = subpath->pathkeys;
    limitPath->subpath = subpath;
    limitPath->limitOffset = limitOffset;
    limitPath->limitCount = limitCount;

    // Adjust the output rows count and costs according to the offset/limit.
    adjust_limit_rows_costs(&limitPath->path.rows,
                            &limitPath->path.startup_cost,
                            &limitPath->path.total_cost,
                            offset_est,
                            count_est);
