...
get_relation_info(PlannerInfo *root, Oid relationObjectId, bool inhparent,
    ...
    if (hasindex) {
        ...
        foreach(l, indexoidlist) {
            ...
            indexOptInfo = makeNode(IndexOptInfo);

            indexOptInfo->indexoid = index->indexrelid;
            indexOptInfo->reltablespace = RelationGetForm(indexRelation)->reltablespace;
            indexOptInfo->rel = rel;
            indexOptInfo->ncolumns = ncolumns = index->indnatts;
            indexOptInfo->nkeycolumns = nkeycolumns = index->indnkeyatts;
            ...
            for (i = 0; i < ncolumns; i++) {
                indexOptInfo->indexkeys[i] = index->indkey.values[i];
                indexOptInfo->canreturn[i] = index_can_return(indexRelation, i + 1);
            }

            for (i = 0; i < nkeycolumns; i++) {
                indexOptInfo->opfamily[i] = indexRelation->rd_opfamily[i];
                indexOptInfo->opcintype[i] = indexRelation->rd_opcintype[i];
                indexOptInfo->indexcollations[i] = indexRelation->rd_indcollation[i];
            }

            indexOptInfo->relam = indexRelation->rd_rel->relam;

            /* We copy just the fields we need, not all of rd_indam */
            amroutine = indexRelation->rd_indam;
            indexOptInfo->amcanorderbyop = amroutine->amcanorderbyop;
            indexOptInfo->amoptionalkey = amroutine->amoptionalkey;
            indexOptInfo->amsearcharray = amroutine->amsearcharray;
            indexOptInfo->amsearchnulls = amroutine->amsearchnulls;
            indexOptInfo->amcanparallel = amroutine->amcanparallel;
            indexOptInfo->amhasgettuple = (amroutine->amgettuple != NULL);
            indexOptInfo->amhasgetbitmap = amroutine->amgetbitmap != NULL &&
                                           relation->rd_tableam->scan_bitmap_next_block != NULL;
            indexOptInfo->amcanmarkpos = (amroutine->ammarkpos != NULL &&
                                          amroutine->amrestrpos != NULL);
            indexOptInfo->amcostestimate = amroutine->amcostestimate;
            Assert(indexOptInfo->amcostestimate != NULL);
            ...
            if (indexOptInfo->relam == BTREE_AM_OID) {
                ...
                indexOptInfo->sortopfamily = indexOptInfo->opfamily;
                indexOptInfo->reverse_sort = (bool *) palloc(sizeof(bool) * nkeycolumns);
                indexOptInfo->nulls_first = (bool *) palloc(sizeof(bool) * nkeycolumns);

                for (i = 0; i < nkeycolumns; i++) {
                    int16 opt = indexRelation->rd_indoption[i];

                    indexOptInfo->reverse_sort[i] = (opt & INDOPTION_DESC) != 0;
                    indexOptInfo->nulls_first[i] = (opt & INDOPTION_NULLS_FIRST) != 0;
                }
                 ...
                 */
                indexOptInfo->sortopfamily = (Oid *) palloc(sizeof(Oid) * nkeycolumns);
                indexOptInfo->reverse_sort = (bool *) palloc(sizeof(bool) * nkeycolumns);
                indexOptInfo->nulls_first = (bool *) palloc(sizeof(bool) * nkeycolumns);
                ...
                for (i = 0; i < nkeycolumns; i++) {
                    ...
                    ltopr = get_opfamily_member(indexOptInfo->opfamily[i],
                                                indexOptInfo->opcintype[i],
                                                indexOptInfo->opcintype[i],
                    ...
                    if (OidIsValid(ltopr) &&
                        get_ordering_op_properties(ltopr,
                                                   ...
                                                   &btopcintype,
                                                   &btstrategy) &&
                        btopcintype == indexOptInfo->opcintype[i] &&
                        btstrategy == BTLessStrategyNumber) {
                        /* Successful mapping */
                        indexOptInfo->sortopfamily[i] = btopfamily;
                    } else {
                        /* Fail ... quietly treat index as unordered */
                        indexOptInfo->sortopfamily = NULL;
                        indexOptInfo->reverse_sort = NULL;
                        indexOptInfo->nulls_first = NULL;
                        break;
                    }
                }
            } else {
             ...
             */
            indexOptInfo->indexprs = RelationGetIndexExpressions(indexRelation);
            indexOptInfo->indpred = RelationGetIndexPredicate(indexRelation);
            if (indexOptInfo->indexprs && varno != 1)
                ChangeVarNodes((Node *) indexOptInfo->indexprs, 1, varno, 0);
            if (indexOptInfo->indpred && varno != 1)
                ChangeVarNodes((Node *) indexOptInfo->indpred, 1, varno, 0);

            /* Build targetlist using the completed indexprs data */
            indexOptInfo->indextlist = build_index_tlist(root, indexOptInfo, relation);

            indexOptInfo->indrestrictinfo = NIL;    /* set later, in indxpath.c */
            indexOptInfo->predOK = false;    /* set later, in indxpath.c */
            indexOptInfo->unique = index->indisunique;
            indexOptInfo->immediate = index->indimmediate;
            indexOptInfo->hypothetical = false;
            ...
            if (indexOptInfo->indpred == NIL) {
                indexOptInfo->pages = RelationGetNumberOfBlocks(indexRelation);
                indexOptInfo->tuples = rel->tuples;
            } else {
                double allvisfrac; /* dummy */

                estimate_rel_size(indexRelation, NULL,
                                  &indexOptInfo->pages, &indexOptInfo->tuples, &allvisfrac);
                if (indexOptInfo->tuples > rel->tuples)
                    indexOptInfo->tuples = rel->tuples;
            }

            if (indexOptInfo->relam == BTREE_AM_OID) {
                /* For btrees, get tree height while we have the index open */
                indexOptInfo->tree_height = _bt_getrootheight(indexRelation);
            } else {
                /* For other index types, just set it to "unknown" for now */
                indexOptInfo->tree_height = -1;
            }

            index_close(indexRelation, NoLock);

            indexinfos = lcons(indexOptInfo, indexinfos);
        }
