...
static Query *transformSelectStmt(ParseState *parseState, SelectStmt *selectStmt);
...
Query *parse_analyze(RawStmt *parseTree,
    ...
    if (numParams > 0) {
        parse_fixed_parameters(parseState, paramTypes, numParams);
    }

    parseState->p_queryEnv = queryEnv;

    Query *query = transformTopLevelStmt(parseState, parseTree);
 ...
 */
Query *transformTopLevelStmt(ParseState *parseState, RawStmt *parseTree) {
    // we're at top level, so allow SELECT INTO
    Query *result = transformOptionalSelectInto(parseState, parseTree->stmt);
...
static Query *transformOptionalSelectInto(ParseState *pstate, Node *parseTree) {
    if (IsA(parseTree, SelectStmt)) {
        ...
        while (selectStmt && selectStmt->op != SETOP_NONE) {
            selectStmt = selectStmt->larg;
        }

        Assert(selectStmt && IsA(selectStmt, SelectStmt) && selectStmt->larg == NULL);

        if (selectStmt->intoClause) {
            CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);

            ctas->query = parseTree;
            ctas->into = selectStmt->intoClause;
    ...
    switch (nodeTag(parseTree)) {
        ...
        case T_SelectStmt: {
            ...
            if (selectStmt->valuesLists) {
                result = transformValuesClause(pstate, selectStmt);
            } else if (selectStmt->op == SETOP_NONE) {
                result = transformSelectStmt(pstate, selectStmt);
            } else {
                result = transformSetOperationStmt(pstate, selectStmt);
            }
        }
            break;
            // Special cases
 ...
 *
 * Note: this covers only cases with no set operations and no VALUES lists; see below for the other cases.
 */
static Query *transformSelectStmt(ParseState *parseState, SelectStmt *selectStmt) {
    ...
    if (selectStmt->withClause) {
        query->hasRecursive = selectStmt->withClause->recursive;
        query->cteList = transformWithClause(parseState, selectStmt->withClause);
        query->hasModifyingCTE = parseState->p_hasModifyingCTE;
    }
    ...
    if (selectStmt->intoClause) {
        ereport(ERROR,
                (errcode(ERRCODE_SYNTAX_ERROR),
                        ...
                        parser_errposition(parseState, exprLocation((Node *) selectStmt->intoClause))));
    }

    // make FOR UPDATE/FOR SHARE info available to addRangeTableEntry */
    parseState->p_locking_clause = selectStmt->lockingClause;

    /* make WINDOW info available for window functions, too */
    parseState->p_windowdefs = selectStmt->windowClause;

    /* process the FROM clause */
    transformFromClause(parseState, selectStmt->fromClause);

    /* transform targetlist */
    query->targetList = transformTargetList(parseState, selectStmt->targetList,
                                            EXPR_KIND_SELECT_TARGET);

    /* mark column origins */
    markTargetListOrigins(parseState, query->targetList);

    /* transform WHERE */
    qual = transformWhereClause(parseState, selectStmt->whereClause, EXPR_KIND_WHERE, "WHERE");

    /* initial processing of HAVING clause is much like WHERE clause */
    query->havingQual = transformWhereClause(parseState, selectStmt->havingClause, EXPR_KIND_HAVING, "HAVING");
     ...
     * them by reference.
     */
    query->sortClause = transformSortClause(parseState,
                                            selectStmt->sortClause,
                                            &query->targetList,
                                            EXPR_KIND_ORDER_BY,
                                            false /* allow SQL92 rules */ );

    query->groupClause = transformGroupClause(parseState,
    ...
    if (selectStmt->distinctClause == NIL) {
    ...
    } else if (linitial(selectStmt->distinctClause) == NULL) {
        /* We had SELECT DISTINCT */
        query->distinctClause = transformDistinctClause(parseState,
                                                        &query->targetList,
                                                        query->sortClause,
                                                        false);
        query->hasDistinctOn = false;
        ...
        query->distinctClause = transformDistinctOnClause(parseState,
    ...
    /* transform LIMIT */
    query->limitOffset = transformLimitClause(parseState, selectStmt->limitOffset, EXPR_KIND_OFFSET, "OFFSET");
    query->limitCount = transformLimitClause(parseState, selectStmt->limitCount, EXPR_KIND_LIMIT, "LIMIT");

    /* transform window clauses after we have seen all window functions */
    query->windowClause = transformWindowDefinitions(parseState,
                                                     parseState->p_windowdefs,
                                                     &query->targetList);

    /* resolve any still-unresolved output columns as being type text */
    if (parseState->p_resolve_unknowns) {
        resolveTargetListUnknowns(parseState, query->targetList);
    }

    query->rtable = parseState->p_rtable;
    query->jointree = makeFromExpr(parseState->p_joinlist, qual);

    query->hasSubLinks = parseState->p_hasSubLinks;
    query->hasWindowFuncs = parseState->p_hasWindowFuncs;
    query->hasTargetSRFs = parseState->p_hasTargetSRFs;
    query->hasAggs = parseState->p_hasAggs;

    foreach(l, selectStmt->lockingClause) {
        transformLockingClause(parseState, query, (LockingClause *) lfirst(l), false);
    }

    assign_query_collations(parseState, query);

    /* this must be done after collations, for reliable comparison of exprs */
    if (parseState->p_hasAggs || query->groupClause || query->groupingSets || query->havingQual)
        parseCheckAggregates(parseState, query);
