...
static Node *transformFuncCall(ParseState *parseState, FuncCall *funcCall);
...
static Node *transformTypeCast(ParseState *pstate, TypeCast *typeCast);
 ...
 */
Node *transformExpr(ParseState *parseState, Node *node, ParseExprKind parseExprKind) {
    /* Save and restore identity of expression type we're parsing */
    Assert(parseExprKind != EXPR_KIND_NONE);

    ParseExprKind sv_expr_kind = parseState->p_expr_kind;

    parseState->p_expr_kind = parseExprKind;
    Node *result = transformExprRecurse(parseState, node);

    parseState->p_expr_kind = sv_expr_kind;
        ...
        case T_SetToDefault:
            ereport(ERROR,
                    (errcode(ERRCODE_SYNTAX_ERROR),
                            ...
                            parser_errposition(pstate, ((SetToDefault *) expr)->location)));
            break;
            /*
...
static Node *transformFuncCall(ParseState *parseState, FuncCall *funcCall) {
    Node *last_srf = parseState->p_last_srf;
    ...
    foreach(arg, funcCall->args) {
        argList = lappend(argList, transformExprRecurse(parseState, (Node *) lfirst(arg)));
    }
     ...
     */
    if (funcCall->agg_within_group) {
        Assert(funcCall->agg_order != NIL);

        foreach(arg, funcCall->agg_order) {
            SortBy *sortBy = (SortBy *) lfirst(arg);
            argList = lappend(argList, transformExpr(parseState, sortBy->node, EXPR_KIND_ORDER_BY));
        }
    }

    /* ... and hand off to ParseFuncOrColumn */
    return ParseFuncOrColumn(parseState,
                             funcCall->funcname,
                             argList,
                             last_srf,
                             funcCall,
...
static Node *transformTypeCast(ParseState *pstate, TypeCast *typeCast) {
    ...
    typenameTypeIdAndMod(pstate, typeCast->typeName, &targetType, &targetTypmod);
    ...
    if (location < 0)
        location = typeCast->typeName->location;
