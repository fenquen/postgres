...
Node *ParseFuncOrColumn(ParseState *pstate,
     ...
     */
    if (list_length(funcArgList) > FUNC_MAX_ARGS)
        ereport(ERROR,
     ...
     */
    nargs = 0;
    for (l = list_head(funcArgList); l != NULL; l = nextl) {
        Node *arg = lfirst(l);
        ...
        if (argtype == VOIDOID && IsA(arg, Param) &&
            ...
            funcArgList = list_delete_ptr(funcArgList, arg);
            continue;
        }
    ...
    if (funcArgList) {
        first_arg = linitial(funcArgList);
    ...
    fdresult = func_get_detail(funcNameList, funcArgList, argnames, nargs,
                               actual_arg_types,
                               !func_variadic, true,
        ...
        ereport(ERROR,
                (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                        errmsg("%s is not a procedure",
                               func_signature_string(funcNameList, nargs, argnames, actual_arg_types)),
    ...
    if (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_PROCEDURE || fdresult == FUNCDETAIL_COERCION) {
        // in these cases, complain if there was anything indicating it must be an aggregate or window function.
        if (agg_star)
            ereport(ERROR,
                    (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                            errmsg("%s(*) specified, but %s is not an aggregate function",
                                   NameListToString(funcNameList),NameListToString(funcNameList)),
                            parser_errposition(pstate, location)));
        if (agg_distinct)
            ereport(ERROR,
                    (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                            errmsg("DISTINCT specified, but %s is not an aggregate function",
                                   NameListToString(funcNameList)),
                            parser_errposition(pstate, location)));
        if (agg_within_group)
            ereport(ERROR,
                    (errcode(ERRCODE_WRONG_OBJECT_TYPE),
    ...
    if (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_PROCEDURE) {
        ...
        if (AGGKIND_IS_ORDERED_SET(aggkind)) {
                    ...
                    ereport(ERROR,
                            (errcode(ERRCODE_UNDEFINED_FUNCTION),
                                    errmsg("function %s does not exist",
                                           func_signature_string(funcNameList, nargs,
                                                                 argnames,
                                                                 actual_arg_types)),
            ...
            if (aggkind == AGGKIND_HYPOTHETICAL)
                unify_hypothetical_args(pstate, funcArgList, numAggregatedArgs,
         ...
         */
        return coerce_type(pstate, linitial(funcArgList),
            ...
            ereport(ERROR,
                    (errcode(ERRCODE_AMBIGUOUS_FUNCTION),
                            errmsg("procedure %s is not unique",
                                   func_signature_string(funcNameList, nargs, argnames,
                                                         actual_arg_types)),
    ...
    make_fn_arguments(pstate, funcArgList, actual_arg_types, declared_arg_types);
    ...
    if (nvargs > 0 && vatype != ANYOID) {
        ...
        vargs = list_copy_tail(funcArgList, non_var_args);
        funcArgList = list_truncate(funcArgList, non_var_args);
        ...
        funcArgList = lappend(funcArgList, newa);
            ...
            ereport(ERROR,
                    (errcode(ERRCODE_DATATYPE_MISMATCH),
                            ...
                            parser_errposition(pstate,
                                               exprLocation((Node *) llast(funcArgList)))));
    }
    ...
    if (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_PROCEDURE) {
         ...
         */
        if (funcArgList == NIL && !agg_star && !agg_within_group)
            ereport(ERROR,
                    (errcode(ERRCODE_WRONG_OBJECT_TYPE),
        ...
        transformAggregateCall(pstate, aggref, funcArgList, agg_order, agg_distinct);
         ...
         * syntax.  This is mere pedantry but some folks insisted ...
         */
        if (wfunc->winagg && funcArgList == NIL && !agg_star)
            ereport(ERROR,
                    (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                            errmsg("%s(*) must be used to call a parameterless aggregate function",
                                   NameListToString(funcNameList)),
