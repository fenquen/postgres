 ...
 */
bool ForwardSyncRequest(const FileTag *fileTag, SyncRequestType syncRequestType) {
    ...
    /*
     * If the checkpointer isn't running or the checkpointerRequest queue is full, the
     * backend will have to perform its own fsync checkpointerRequest.  But before forcing
     * that to happen, we can try to compact the checkpointerRequest queue.
     */
    if (CheckpointerShmem->checkpointer_pid == 0 ||
        (CheckpointerShmem->num_requests >= CheckpointerShmem->max_requests && !CompactCheckpointerRequestQueue())) {

        // Count the subset of writes where backends have to do their own fsync
        if (!AmBackgroundWriterProcess()) {
    ...
    /* OK, insert checkpointerRequest */
    checkpointerRequest = &CheckpointerShmem->requests[CheckpointerShmem->num_requests++];
    checkpointerRequest->ftag = *fileTag;
    checkpointerRequest->type = syncRequestType;
