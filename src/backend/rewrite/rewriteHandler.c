...
markQueryForLocking(Query *qry, Node *jtnode,
    ...
    if (IsA(jtnode, RangeTblRef)) {
        ...
        foreach(l, f->fromlist) markQueryForLocking(qry, lfirst(l), strength, waitPolicy, pushedDown);
 ...
 *	  Rewrite one query via query rewrite system, possibly returning 0 or many queries.
 *
