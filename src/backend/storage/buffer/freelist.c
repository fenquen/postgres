 ...
 * being read.
 */
int StrategySyncStart(uint32 *completeNBufferRoundNum, uint32 *allocatedBufferNum) {
    ...
    if (completeNBufferRoundNum) {
        *completeNBufferRoundNum = StrategyControl->completePasses;

        /*
         * Additionally add the number of wraparounds that happened before
         * completePasses could be incremented. C.f. ClockSweepTick().
         */
        *completeNBufferRoundNum += nextVictimBuffer / NBuffers;
    }

    if (allocatedBufferNum) {
        *allocatedBufferNum = pg_atomic_exchange_u32(&StrategyControl->numBufferAllocs, 0);
    }
