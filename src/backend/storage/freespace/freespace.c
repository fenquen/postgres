...
/*
 * We use just 额外的1个字节 to store the amount of free space on a page, so we
 ...
 */
#define FSM_CATEGORIES    256 // 对应1字节的表示范围
...
#define FSM_ROOT_LEVEL    (FSM_TREE_DEPTH - 1) // 说是level其实是height意思 root到bottom到level是由大到小的
...
static BlockNumber fsm_search(Relation relation, uint8 minCat);
 ...
 * extend the relation.
 */
BlockNumber GetPageWithFreeSpace(Relation relation, Size spaceNeeded) {
    uint8 minCat = fsm_space_needed_to_cat(spaceNeeded);

    return fsm_search(relation, minCat);
}
...
// return the lower bound of the range of free space represented by given category.
static Size fsm_space_cat_to_avail(uint8 cat) {
 ...
 */
static uint8 fsm_space_needed_to_cat(Size needed) {
    // can't ask for more space than the highest category represent
...
// search the tree for a heap page with at least minCat of free space
static BlockNumber fsm_search(Relation relation, uint8 minCat) {
    int restarts = 0;
    FSMAddress fsmAddress = FSM_ROOT_ADDRESS;
    ...
    for (;;) {
        ...
        // read the FSM page
        Buffer buffer = fsm_readbuf(relation, fsmAddress, false);

        // search within the page
        if (BufferIsValid(buffer)) {
            LockBuffer(buffer, BUFFER_LOCK_SHARE);

            slot = fsm_search_avail(buffer,
                                    minCat,
                                    (fsmAddress.level == FSM_BOTTOM_LEVEL),
                                    false);

            if (slot == -1) {
                maxAvail = fsm_get_max_avail(BufferGetPage(buffer));
            }
        ...
        if (slot != -1) {
            ...
            if (fsmAddress.level == FSM_BOTTOM_LEVEL) {
                return fsm_get_heap_blk(fsmAddress, slot);
            }

            fsmAddress = fsm_get_child(fsmAddress, slot);
        } else if (fsmAddress.level == FSM_ROOT_LEVEL) {
            // at the root failure means there's no page with enough free space in the FSM. Give up.
             ...
             * node didn't reflect the value on the lower page. Update
             * the upper node, to avoid falling into the same trap again, and
             * start over.
             *
             * There's a race condition here, if another backend updates this
             * page right after we release it, and gets the lock on the parentFSMAddress
             * page before us. We'll then update the parentFSMAddress page with the now
             * stale information we had. It's OK, because it should happen
             * rarely, and will be fixed by the next vacuum.
             */
            FSMAddress parentFSMAddress = fsm_get_parent(fsmAddress, &parentSlot);
            fsm_set_and_search(relation, parentFSMAddress, parentSlot, maxAvail, 0);
