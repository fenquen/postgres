...
typedef struct _MdfdVec {
    File mdfd_vfd;        /* fd number in fd.c's pool ,segment文件对应的fd */
...
static MdfdVec *mdopen(SMgrRelation sMgrRelation, ForkNumber forknum, int behavior);

static void register_dirty_segment(SMgrRelation sMgrRelation, ForkNumber forknum,
                                   MdfdVec *seg);
...
static char *_mdfd_segpath(SMgrRelation sMgrRelation, ForkNumber forknum,
                           BlockNumber segno);

static MdfdVec *_mdfd_openseg(SMgrRelation sMgrRelation, ForkNumber forkNumber,
                              BlockNumber segno, int oflags);

static MdfdVec *_mdfd_getseg(SMgrRelation sMgrRelation, ForkNumber forkNumber,
                             BlockNumber blockNumber, bool skipFsync, int behavior);
...
bool mdexists(SMgrRelation reln, ForkNumber forkNum) {
    // Close it first, to ensure that we notice if the fork has been unlinked since we opened it.
 ...
 *		extending a relation (i.e., blockNumber is at or beyond the current
 *		EOF).  Note that we assume writing a block beyond current EOF
 *		causes intervening file space to become filled with zeroes.
 */
void mdextend(SMgrRelation sMgrRelation,
    ...
    Assert(blockNumber >= mdnblocks(sMgrRelation, forkNumber));
#endif

    /*
     * If a relation manages to grow to 2^32-1 blocks, refuse to extend it
     * we mustn't create a block whose number actually is
        ...
        ereport(ERROR,
                (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                        errmsg("cannot extend file \"%s\" beyond %u blocks",
                               relpath(sMgrRelation->smgr_rnode, forkNumber),
                               InvalidBlockNumber)));
    }

    mdfdVec = _mdfd_getseg(sMgrRelation, forkNumber, blockNumber, skipFsync, EXTENSION_CREATE);

    offset = (off_t) BLCKSZ * (blockNumber % ((BlockNumber) RELSEG_SIZE));

    Assert(offset < (off_t) BLCKSZ * RELSEG_SIZE);

    if ((nbytes = FileWrite(mdfdVec->mdfd_vfd, buffer, BLCKSZ, offset, WAIT_EVENT_DATA_FILE_EXTEND)) != BLCKSZ) {
        if (nbytes < 0) {
            ereport(ERROR,
                    (errcode_for_file_access(),
                            errmsg("could not extend file \"%s\": %m", FilePathName(mdfdVec->mdfd_vfd)),
        ...
        ereport(ERROR,
                (errcode(ERRCODE_DISK_FULL),
                        errmsg("could not extend file \"%s\": wrote only %d of %d bytes at block %u",
                               FilePathName(mdfdVec->mdfd_vfd), nbytes, BLCKSZ, blockNumber),
                        errhint("Check free disk space.")));
    }

    if (!skipFsync && !SmgrIsTemp(sMgrRelation)) {
        register_dirty_segment(sMgrRelation, forkNumber, mdfdVec);
    }

    Assert(_mdnblocks(sMgrRelation, forkNumber, mdfdVec) <= ((BlockNumber) RELSEG_SIZE));
}
...
static MdfdVec *mdopen(SMgrRelation sMgrRelation, ForkNumber forknum, int behavior) {
    ...
    if (sMgrRelation->md_num_open_segs[forknum] > 0) {
        return &sMgrRelation->md_seg_fds[forknum][0];
    }

    path = relpath(sMgrRelation->smgr_rnode, forknum);
    ...
    _fdvec_resize(sMgrRelation, forknum, 1);
    mdfdVec = &sMgrRelation->md_seg_fds[forknum][0];
    mdfdVec->mdfd_vfd = fd;
    mdfdVec->mdfd_segno = 0;

    Assert(_mdnblocks(sMgrRelation, forknum, mdfdVec) <= ((BlockNumber) RELSEG_SIZE));
...
void mdwriteback(SMgrRelation reln, ForkNumber forknum,
    ...
    while (nblocks > 0) {
        ...
        // We might be flushing buffers of already removed relations, that's ok, just ignore that case.
        if (!v) {
            return;
        }
...
void mdread(SMgrRelation sMgrRelation,
            ForkNumber forkNumber,
            BlockNumber blockNumber,
            char *buffer) {
    off_t offset;
    ...
    TRACE_POSTGRESQL_SMGR_MD_READ_START(forkNumber, blockNumber,
    ...
    mdfdVec = _mdfd_getseg(sMgrRelation,
                           forkNumber,
                           blockNumber,
                           false,
    ...
    // 文件的指定的offset读取BLCKSZ字节注入到buffer
    byteNum = FileRead(mdfdVec->mdfd_vfd, buffer, BLCKSZ, offset, WAIT_EVENT_DATA_FILE_READ);

    TRACE_POSTGRESQL_SMGR_MD_READ_DONE(forkNumber, blockNumber,
                                       sMgrRelation->smgr_rnode.node.spcNode,
                                       sMgrRelation->smgr_rnode.node.dbNode,
                                       sMgrRelation->smgr_rnode.node.relNode,
                                       sMgrRelation->smgr_rnode.backend,
    ...
    if (byteNum != BLCKSZ) {
        if (byteNum < 0) {
            ereport(ERROR,
                    (errcode_for_file_access(),
                            errmsg("could not read block %u in file \"%s\": %m", blockNumber,
                                   FilePathName(mdfdVec->mdfd_vfd))));
        }
            ...
            ereport(ERROR,
                    (errcode(ERRCODE_DATA_CORRUPTED),
                            errmsg("could not read block %u in file \"%s\": read only %d of %d bytes",
                                   blockNumber, FilePathName(mdfdVec->mdfd_vfd), byteNum, BLCKSZ)));
        }
    }
}
 ...
 *		use mdextend().
 */
void mdwrite(SMgrRelation sMgrRelation,
             ForkNumber forknum,
    ...
    /* This assert is too expensive to have on normally ... */
#ifdef CHECK_WRITE_VS_EXTEND
    Assert(blocknum < mdnblocks(sMgrRelation, forknum));
#endif
    ...
    mdfdVec = _mdfd_getseg(sMgrRelation, forknum, blocknum, skipFsync, EXTENSION_FAIL | EXTENSION_CREATE_RECOVERY);

    offest = (off_t) BLCKSZ * (blocknum % ((BlockNumber) RELSEG_SIZE));

    Assert(offest < (off_t) BLCKSZ * RELSEG_SIZE);

    nbytes = FileWrite(mdfdVec->mdfd_vfd, buffer, BLCKSZ, offest, WAIT_EVENT_DATA_FILE_WRITE);
            ...
            ereport(ERROR,
                    (errcode_for_file_access(),
                            errmsg("could not write block %u in file \"%s\": %m",
                                   blocknum, FilePathName(mdfdVec->mdfd_vfd))));
        /* short write: complain appropriately */
        ereport(ERROR, (errcode(ERRCODE_DISK_FULL),
                        errmsg("could not write block %u in file \"%s\": wrote only %d of %d bytes",
                               blocknum, FilePathName(mdfdVec->mdfd_vfd), nbytes, BLCKSZ),
                               errhint("Check free disk space.")));
    }

    if (!skipFsync && !SmgrIsTemp(sMgrRelation)) {
        register_dirty_segment(sMgrRelation, forknum, mdfdVec);
    }
}
 ...
 */
static void register_dirty_segment(SMgrRelation sMgrRelation, ForkNumber forknum, MdfdVec *seg) {
    FileTag fileTag;

    INIT_MD_FILETAG(fileTag, sMgrRelation->smgr_rnode.node, forknum, seg->mdfd_segno);

    /* Temp relations should never be fsync'd */
    Assert(!SmgrIsTemp(sMgrRelation));

    if (!RegisterSyncRequest(&fileTag, SYNC_REQUEST, false /* retryOnError */ )) {
...
// return the filename for the specified segment of the relation. The returned string is palloc'd.
static char *_mdfd_segpath(SMgrRelation sMgrRelation, ForkNumber forknum, BlockNumber segno) {
    char *path, *fullpath;
 ...
 */
static MdfdVec *_mdfd_openseg(SMgrRelation sMgrRelation,
                              ForkNumber forkNumber,
                              BlockNumber segno,
                              int oflags) {
    MdfdVec *mdfdVec;
    ...
    // 对应的数据文件的path
    fullpath = _mdfd_segpath(sMgrRelation, forkNumber, segno);
    ...
    if (segno <= sMgrRelation->md_num_open_segs[forkNumber]) {
        _fdvec_resize(sMgrRelation, forkNumber, segno + 1);
    }

    /* fill the entry */
    mdfdVec = &sMgrRelation->md_seg_fds[forkNumber][segno];
    mdfdVec->mdfd_vfd = fd;
    mdfdVec->mdfd_segno = segno;
 ...
 */
static MdfdVec *_mdfd_getseg(SMgrRelation sMgrRelation,
                             ForkNumber forkNumber,
                             BlockNumber blockNumber,
                             bool skipFsync,
                             int behavior) {
    ...
    // blockNumber是在表的global维度上的 不是某个segment下的
    targetseg = blockNumber / ((BlockNumber) RELSEG_SIZE);

    // if an existing and opened segment, we're done
    if (targetseg < sMgrRelation->md_num_open_segs[forkNumber]) {
        mdfdVec = &sMgrRelation->md_seg_fds[forkNumber][targetseg];
        return mdfdVec;
    }
     ...
     */
    if (sMgrRelation->md_num_open_segs[forkNumber] > 0) {
        mdfdVec = &sMgrRelation->md_seg_fds[forkNumber][sMgrRelation->md_num_open_segs[forkNumber] - 1];
    } else {
        mdfdVec = mdopen(sMgrRelation, forkNumber, behavior);
        if (!mdfdVec) { /* if behavior & EXTENSION_RETURN_NULL */
            return NULL;
        }
    }

    // iterate over all the segment between the last opened and the target segment.
    for (nextsegno = sMgrRelation->md_num_open_segs[forkNumber]; nextsegno <= targetseg; nextsegno++) {
        // 当前的这个数据文件大小包含多少
        BlockNumber nblocks = _mdnblocks(sMgrRelation, forkNumber, mdfdVec);
        int flags = 0;

        Assert(nextsegno == mdfdVec->mdfd_segno + 1);
            ...
            ereport(ERROR,
                    (errcode_for_file_access(),
                            errmsg("could not open file \"%s\" (target block %u): previous segment is only %u blocks",
                                   _mdfd_segpath(sMgrRelation, forkNumber, nextsegno), blockNumber, nblocks)));
        }

        // open 该segment对应的文件
        mdfdVec = _mdfd_openseg(sMgrRelation, forkNumber, nextsegno, flags);
        if (mdfdVec == NULL) {
            if ((behavior & EXTENSION_RETURN_NULL) && FILE_POSSIBLY_DELETED(errno)) {
                return NULL;
            }
            ...
            ereport(ERROR,
                    (errcode_for_file_access(),
                            errmsg("could not open file \"%s\" (target block %u): %m",
                                   _mdfd_segpath(sMgrRelation, forkNumber, nextsegno), blockNumber)));
        }
    }
...
static BlockNumber _mdnblocks(SMgrRelation reln,
                              ForkNumber forknum,
                              MdfdVec *mdfdVec) {
    off_t len = FileSize(mdfdVec->mdfd_vfd);
    if (len < 0) {
        ereport(ERROR,
                (errcode_for_file_access(), errmsg("could not seek to end of file \"%s\": %m",
                                                   FilePathName(mdfdVec->mdfd_vfd))));
    }
