 ...
 */
List *pg_parse_query(const char *queryString) {
    List *rawParseTreeList;

    TRACE_POSTGRESQL_QUERY_PARSE_START(queryString);

    if (log_parser_stats) {
        ResetUsage();
    }

    rawParseTreeList = raw_parser(queryString);
    ...
    if (log_parser_stats) {
        ...
        {
            List	   *new_list = copyObject(rawParseTreeList);

            /* This checks both copyObject() and the equal() routines... */
            if (!equal(new_list, rawParseTreeList))
                elog(WARNING, "copyObject() failed to produce an equal raw parse tree");
            else
                rawParseTreeList = new_list;
        }
#endif

    /*
     * Currently, outfuncs/readfuncs support is missing for many raw parse
     * tree nodes, so we don't try to implement WRITE_READ_PARSE_PLAN_TREES here.
     */
    TRACE_POSTGRESQL_QUERY_PARSE_DONE(query_string);

    return rawParseTreeList;
}
...
List *pg_analyze_and_rewrite(RawStmt *parseTree,
    ...
    TRACE_POSTGRESQL_QUERY_REWRITE_START(queryString);
    ...
    Query *query = parse_analyze(parseTree, queryString, paramTypes, numParams, queryEnv);
    ...
    List *queryTreeList = pg_rewrite_query(query);
...
List *pg_analyze_and_rewrite_params(RawStmt *parsetree,
    ...
    parseState->p_sourcetext = query_string;
    parseState->p_queryEnv = queryEnv;
    (*parserSetup)(parseState, parserSetupArg);

    query = transformTopLevelStmt(parseState, parsetree);

    if (post_parse_analyze_hook)
        (*post_parse_analyze_hook)(parseState, query);
...
static List *pg_rewrite_query(Query *query) {
    ...
    if (query->commandType == CMD_UTILITY) {
        queryList = list_make1(query);
    } else { // rewrite regular queries */
    ...
    if (Debug_print_rewritten) {
        elog_node_display(LOG, "rewritten parse tree", queryList, Debug_pretty_print);
    }
 ...
 */
PlannedStmt *pg_plan_query(Query *query,
                           int cursorOptions,
    ...
    // call the optimizer
    PlannedStmt *plannedStmt = planner(query, cursorOptions, boundParams);
...
#ifdef COPY_PARSE_PLAN_TREES
    /* Optional debugging check: pass plannedStmt tree through copyObject() */
    {
        PlannedStmt *new_plan = copyObject(plannedStmt);
...
#ifdef NOT_USED
        /* This checks both copyObject() and the equal() routines... */
        if (!equal(new_plan, plannedStmt))
            elog(WARNING, "copyObject() failed to produce an equal plannedStmt tree");
        else
#endif
...
#ifdef WRITE_READ_PARSE_PLAN_TREES
    /* Optional debugging check: pass plannedStmt tree through outfuncs/readfuncs */
    {
        char	   *str;
        PlannedStmt *new_plan;

        str = nodeToString(plannedStmt);
        ...
        /* This checks both outfuncs/readfuncs and the equal() routines... */
        if (!equal(new_plan, plannedStmt))
            elog(WARNING, "outfuncs/readfuncs failed to produce an equal plannedStmt tree");
        else
#endif
            plannedStmt = new_plan;
    }
    ...
    // print plannedStmt if debugging.
    if (Debug_print_plan) {
        elog_node_display(LOG, "plannedStmt", plannedStmt, Debug_pretty_print);
    }
 ...
 *
 * For normal optimizable statements, invoke the planner.
 * For utility statements, just make a wrapper PlannedStmt node.
 *
 * The result is a list of PlannedStmt
 */
List *pg_plan_queries(List *queryList,
                      int cursorOptions,
                      ParamListInfo boundParams) {
    List *plannedStmtList = NIL;

    ListCell *listCell;
    foreach(listCell, queryList) {
        Query *query = lfirst_node(Query, listCell);
        PlannedStmt *plannedStmt;

        // utility command require no planning
        if (query->commandType == CMD_UTILITY) {
            plannedStmt = makeNode(PlannedStmt);
            plannedStmt->commandType = CMD_UTILITY;
            plannedStmt->canSetTag = query->canSetTag;
            plannedStmt->utilityStmt = query->utilityStmt;
            plannedStmt->stmt_location = query->stmt_location;
            plannedStmt->stmt_len = query->stmt_len;
        } else {
            plannedStmt = pg_plan_query(query, cursorOptions, boundParams);
        }

        plannedStmtList = lappend(plannedStmtList, plannedStmt);
    }

    return plannedStmtList;
}


// execute a "simple Query" protocol message.
static void exec_simple_query(const char *queryString) {
    ...
    TRACE_POSTGRESQL_QUERY_START(queryString);

    // we use save_log_statement_stats so ShowUsage doesn't report incorrect result because ResetUsage wasn't called.
     ...
     * queryString will be in this same command block, *unless* we find a
    ...
    if (check_log_statement(parseTreeList)) {
        ereport(LOG, (errmsg("statement: %s", queryString), errhidestmt(true), errdetail_execute(parseTreeList)));
        was_logged = true;
    }
     ...
     * behavior properly in the transaction machinery, we use an "implicit" transaction block.
     */
    bool use_implicit_block = (list_length(parseTreeList) > 1);

    // Run through the raw parse tree(s) and process each one.
    ListCell *listCell;
    foreach(listCell, parseTreeList) {
         ...
         * do any special start-of-SQL-command processing needed by the destination.
         */
        const char *commandTag = CreateCommandTag(parseTree->stmt);// "select"

        set_ps_display(commandTag, false);

        // 空的实现
        BeginCommand(commandTag, commandDest);
        ...
        // Switch to appropriate context for constructing querytrees (again these must outlive the execution context).
        oldMemoryContext = MemoryContextSwitchTo(MessageContext);

        List *queryTree = pg_analyze_and_rewrite(parseTree, queryString, NULL, 0, NULL);
         ...
         * portal anyway. 通过PortalDefineQuery将查询计划等数据设置到Portal上之后，Portal的状态变为PORTAL_DEFINED
         */
        ...
        DestReceiver *destReceiver = CreateDestReceiver(commandDest);
        if (commandDest == DestRemote) {
         ...
         */
        EndCommand(completionTag, commandDest);
    }
    ...
    // there were no parse trees, return EmptyQueryResponse message.
    if (!parseTreeList) {
        NullCommand(commandDest);
    }
...
die(SIGNAL_ARGS) {
     ...
     */
    if (readingCommand && whereToSendOutput != DestRemote)
        ProcessInterrupts();
...
void ProcessInterrupts(void) {
     ...
     * dislodge it.
     */
    if (RecoveryConflictPending && readingCommand) {
...
void PostgresMain(int argc,
    ...
    volatile bool sendReadyForQuery = true;
    bool disableIdleInTransactionTimeout = false;
    ...
    if (am_walsender) {
         ...
         * easily, while SIGTERM cannot, so we make both signals do die() rather than quickdie().
         */
        if (IsUnderPostmaster) {
     ...
     * (to wit, UnBlockSig) will be restored when longjmp to here.  This
     * is essential in case we longjmp'd out of a signal handler on a platform
     * where that leaves the signal blocked.  It's not redundant with the
     * unblock in AbortTransaction() because the latter is only called if we
     * were inside a transaction.
     */
    // https://www.cnblogs.com/nufangrensheng/p/3516134.html
    // 返回的值如果不是0 说明是在别的地点又跳转到这里的通常都是异常处理的时候
    ...
    if (!ignore_till_sync) {
        sendReadyForQuery = true;    /* initially, or after error */
    }
    ...
    for (;;) {
        ...
        if (sendReadyForQuery) {
            if (IsAbortedTransactionBlockState()) {
                ...
                if (IdleInTransactionSessionTimeout > 0) {
                    disableIdleInTransactionTimeout = true;
        ...
        firstChar = ReadCommand(&inputMessage);
         ...
         */
        if (disableIdleInTransactionTimeout) {
            disable_timeout(IDLE_IN_TRANSACTION_SESSION_TIMEOUT, false);
            disableIdleInTransactionTimeout = false;
        }
         ...
         * it's called when readingCommand is set, so check for interrupts
         * before resetting readingCommand.
         */
        CHECK_FOR_INTERRUPTS();
        readingCommand = false;
        ...
        switch (firstChar) {
            case 'Q': { // simple query
                ...
                pq_getmsgend(&inputMessage);

                if (am_walsender) {
                    if (!exec_replication_command(queryString)) {
            ...
            case 'P': { /* parse */
                ...
                stmt_name = pq_getmsgstring(&inputMessage);
                query_string = pq_getmsgstring(&inputMessage);
                numParams = pq_getmsgint(&inputMessage, 2);
                if (numParams > 0) {
                    paramTypes = (Oid *) palloc(numParams * sizeof(Oid));
                    for (int i = 0; i < numParams; i++)
                        paramTypes[i] = pq_getmsgint(&inputMessage, 4);
                }
                pq_getmsgend(&inputMessage);
            ...
            case 'B':            /* bind */
                 ...
                 */
                exec_bind_message(&inputMessage);
                break;
            ...
            case 'E':            /* execute */
            {
                ...
                portal_name = pq_getmsgstring(&inputMessage);
                max_rows = pq_getmsgint(&inputMessage, 4);
                pq_getmsgend(&inputMessage);
            ...
            case 'F':            /* fastpath function call */
                ...
                HandleFunctionRequest(&inputMessage);
            ...
            case 'C':            /* close */
            {
                ...
                close_type = pq_getmsgbyte(&inputMessage);
                close_target = pq_getmsgstring(&inputMessage);
                pq_getmsgend(&inputMessage);
            ...
            case 'D':            /* describe */
            {
                ...
                describe_type = pq_getmsgbyte(&inputMessage);
                describe_target = pq_getmsgstring(&inputMessage);
                pq_getmsgend(&inputMessage);
