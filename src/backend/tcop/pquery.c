...
QueryDesc *CreateQueryDesc(PlannedStmt *plannedstmt,
    ...
    queryDesc->operation = plannedstmt->commandType;    /* operation */
    queryDesc->plannedstmt = plannedstmt;    /* plan */
    queryDesc->sourceText = sourceText;    /* query text */
    queryDesc->snapshot = RegisterSnapshot(snapshot);    /* snapshot */
    /* RI check snapshot */
    queryDesc->crosscheck_snapshot = RegisterSnapshot(crosscheck_snapshot);
    queryDesc->dest = dest;            /* output dest */
    queryDesc->params = params;        /* parameter values passed into query */
    queryDesc->queryEnv = queryEnv;
    queryDesc->instrument_options = instrument_options;    /* instrumentation wanted? */
...
void PortalStart(Portal portal,
    ...
    Portal previousActivePortal = ActivePortal;
    ResourceOwner previousResourceOwner = CurrentResourceOwner;
    MemoryContext previousPortalContext = PortalContext;
    ...
    PG_TRY();
            {
                ...
                switch (portal->strategy) {
                    case PORTAL_ONE_SELECT: // 通常都是
                        ...
                        QueryDesc *queryDesc = CreateQueryDesc(linitial_node(PlannedStmt, portal->stmts),
                                                               portal->sourceText,
                                                               GetActiveSnapshot(),
                                                               InvalidSnapshot,
                                                               None_Receiver,
        ...
        PG_CATCH();
            {
                ...
                ActivePortal = previousActivePortal;
                CurrentResourceOwner = previousResourceOwner;
                PortalContext = previousPortalContext;
...
static uint64 PortalRunSelect(Portal portal,
     ...
     * at the end of the available tuples in that scanDirection.  If so, set the
     * scanDirection to NoMovement to avoid trying to fetch any tuples.  (This
