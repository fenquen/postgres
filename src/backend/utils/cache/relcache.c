...
#define RelationCacheInsert(RELATION, replaceAllowed)    \
do { \
...
static Relation AllocateRelationDesc(Form_pg_class formPgClass);

static void RelationParseRelOptions(Relation relation, HeapTuple tuple);

static void RelationBuildTupleDesc(Relation relation);

static Relation RelationBuildDesc(Oid targetRelationOid, bool insertIntoRelationCache);
...
/*
 *		ScanPgRelation 本质是 select * from pg_class where oid = ?
 *
...
static HeapTuple ScanPgRelation(Oid targetRelId,
    ...
    /*
	 * Open table pg_class and fetch a tuple.  Force heap scan if we haven't yet
	 * built the critical relcache entries (this includes initdb and startup
	 * without a pg_internal.init file).  The caller can also force a heap
	 * scan by setting indexOK == false.
	 */
    Relation pgClassTable = table_open(RelationRelationId, AccessShareLock);
    ...
    SysScanDesc sysScanDescPgClass = systable_beginscan(pgClassTable,
                                                        ClassOidIndexId,
    ...
    HeapTuple pgClassTuple = systable_getnext(sysScanDescPgClass);

    // Must copy tuple before releasing buffer.
    if (HeapTupleIsValid(pgClassTuple)) {
        pgClassTuple = heap_copytuple(pgClassTuple);
    }

    /* all done */
    systable_endscan(sysScanDescPgClass);
    table_close(pgClassTable, AccessShareLock);
...
static Relation AllocateRelationDesc(Form_pg_class formPgClass) {
    ...
    memcpy(relationForm, formPgClass, CLASS_TUPLE_SIZE);
    relation->rd_rel = relationForm; // initialize relation tuple form
...
// Form the relation's tupleDesc from information in the pg_attribute, pg_attrdef & pg_constraint system catalogs.
static void RelationBuildTupleDesc(Relation relation) {
 ...
 *
 *		The new descriptor is inserted into the hash table if insertIntoRelationCache is true.
 *
...
static Relation RelationBuildDesc(Oid targetRelationOid, bool insertIntoRelationCache) {
    ...
    int in_progress_offset = in_progress_list_len++;

    in_progress_list[in_progress_offset].reloid = targetRelationOid;

    retry:
    in_progress_list[in_progress_offset].invalidated = false;

    // select * from pg_class where oid = targetRelationOid
    HeapTuple pgClassTuple = ScanPgRelation(targetRelationOid, true, false);
    ...
    // get information from the pgClassTuple
    Form_pg_class formPgClass = (Form_pg_class) GETSTRUCT(pgClassTuple);

    Oid relid = formPgClass->oid;
    Assert(relid == targetRelationOid);



    // palloc0生成relation, and copy pgClassTuple to relation->rd_rel.
    Relation relation = AllocateRelationDesc(formPgClass);
    ...
    // 如果是分区的表  initialize key and partition descriptor info
    ...
    RelationParseRelOptions(relation, pgClassTuple);

    // initialize the relation lock manager information
    RelationInitLockInfo(relation); /* see lmgr.c */

    // initialize physical addressing information for the relation
    // 注入 relation->rd_node(type是RelFileNode) 各个属性
    ...
    // now we can free the memory allocated for pgClassTuple
	 ...
	 * for the !insertIntoRelationCache case.  For the insertIntoRelationCache case, RelationCacheInsert()
	 ...
	 */
    if (insertIntoRelationCache) {
        // 对应的hashMap是 RelationIdCache ,key是 relation的oid
...
Relation RelationIdGetRelation(Oid relationId) {
    ...
    if (RelationIsValid(relation)) {
        ...
        if (!relation->rd_isvalid) {
			 ...
			 */
            Assert(relation->rd_isvalid || (relation->rd_isnailed && !criticalRelcachesBuilt));
        }

        return relation;
    }

    // no reldesc in the cache, RelationBuildDesc() build one and add it.
    relation = RelationBuildDesc(relationId, true);
    if (RelationIsValid(relation)) {
        RelationIncrementReferenceCount(relation);
    }
