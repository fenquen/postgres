...
void EnablePortalManager(void) {
    ...
    // use PORTALS_PER_USER as a guess of how many hash table entries to create, initially
    PortalHashTable = hash_create("Portal hash", PORTALS_PER_USER, &ctl, HASH_ELEM);
}

// Return a portal given a portal name, or NULL if name not found.
...
/*
 * build a new portal under given name.
 *
 * allowDuplicate: if true, automatically drop any pre-existing portal of the same name (if false, an error is raised).
 * dupSilent: if true, don't even emit a WARNING.
 */
Portal CreatePortal(const char *name, bool allowDuplicate, bool dupSilent) {
...
void PortalDefineQuery(Portal portal,
                       ...
                       List *plannedStmtList,
                       CachedPlan *cachedPlan) {
    AssertArg(PortalIsValid(portal));
    AssertState(portal->status == PORTAL_NEW);

    AssertArg(queryString != NULL);
    AssertArg(commandTag != NULL || plannedStmtList == NIL);

    portal->prepStmtName = prepStmtName;
    portal->sourceText = queryString;
    portal->commandTag = commandTag;
    portal->stmts = plannedStmtList;
    portal->cplan = cachedPlan;
...
void PortalDrop(Portal portal, bool isTopCommit) {
    ...
    // Don't allow dropping a pinned portal, it's still needed by whoever pinned it.
    if (portal->portalPinned)
        ereport(ERROR,
