...
static void ResourceArrayEnlarge(ResourceArray *resourceArray);
...
static void ResourceArrayEnlarge(ResourceArray *resourceArray) {
    ...
    if (resourceArray->nitems < resourceArray->maxitems)
        return;                    /* no work needed */

    olditemsarr = resourceArray->itemsarr;
    oldcap = resourceArray->capacity;
    ...
    for (i = 0; i < newcap; i++)
        newitemsarr[i] = resourceArray->invalidval;

    /* We assume we can't fail below this point, so OK to scribble on resarr */
    resourceArray->itemsarr = newitemsarr;
    resourceArray->capacity = newcap;
    resourceArray->maxitems = RESARRAY_MAX_ITEMS(newcap);
    resourceArray->nitems = 0;
    ...
    if (olditemsarr != NULL) {
         ...
         */
        for (i = 0; i < oldcap; i++) {
            if (olditemsarr[i] != resourceArray->invalidval)
                ResourceArrayAdd(resourceArray, olditemsarr[i]);
        }
    ...
    Assert(resourceArray->nitems < resourceArray->maxitems);
}
