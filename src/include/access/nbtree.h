...
/*
 *	BTPageOpaqueData -- 在 page 末尾, we store a pointer
...
/*
 *	Lehman and Yao's algorithm requires a ``high key'' on every non-rightmost page.
 *
 *	The high key is not a tuple that is used to visit the heap.
 *
 *	It is a pivot tuple (see "Notes on B-Tree tuple format" below for definition).
 *
 *	要点 The high key on a page is required to be greater than or equal to any
 *	other key that appears on the page.
 *
 *	If we find ourselves trying to insert a key that is strictly > high key, we know we need to move right
...
/* Get/set down link block number */
...
extern Buffer _bt_relandgetbuf(Relation indexRelation, Buffer oldBuffer,
                               BlockNumber targetBlockNumber, int lockMode);
...
extern Buffer _bt_moveright(Relation indexRelation, BTScanInsert key, Buffer buffer,
                            bool forupdate, BTStack btStack, int access, Snapshot snapshot);

extern OffsetNumber _bt_binsrch_insert(Relation rel, BTInsertState insertstate);

extern int32 _bt_compare(Relation rel, BTScanInsert key, Page page, OffsetNumber offsetNumber);
